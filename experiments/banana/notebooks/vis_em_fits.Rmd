...
```{r}
gp_prior_mean <- function(U) lpost_em$shift_func(U) - calc_lprior_dens(U, inv_prob$par_prior_trunc)


test <- lpost_em$predict_emulator(input_grid)
plot_2d_grid(input_grid, test$var, input_design)
plot_2d_grid(input_grid, gp_prior_mean(input_grid), input_design)
```
