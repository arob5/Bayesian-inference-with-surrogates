...
lpost <- function(U) {
  ...
  lprior(U) + llik_obj$assemble_llik(U)
}
```
...
lpost_em <- llikEmulatorGP("em_lpost", gp_obj, default_conditional=FALSE, 
                           ...
                           is_lpost_em=TRUE)
saveRDS(lpost_em, file.path(out_dir, "..", "lpost_em.rds"))
```
...
true_func <- function(U) lpost(U) - lpost_eki(U)
...
lpost_prior_eval <- lpost(prior_eval)
resids_lpost_prior_eval <- lpost_prior_eval - lpost_eki(prior_eval)

post_eval <- select_mcmc_samp_mat(samp_dt)
post_eval <- post_eval[sample(1:nrow(post_eval), size=n_eval), inv_prob$par_names]
lpost_post_eval <- lpost(post_eval)
...
```{r}
al_results_maxvar <- run_seq_design(lpost_em, "llik_neg_var_gp", n_batch=n_batch, 
...
plt_maxvar_mae <- ggplot(acq_results[metric=="crps"], aes(x=itr)) + 
