...
An alternative popular strategy is is to approximate the function output as a linear combination of a small number
...
\textit{calibrate, emulate, sample} \citep{CES,idealizedGCM,CESSoftware} workflow, which uses 
...
throughout this review. Extensions to the GP methodology can produce more flexible 
...
BoTorch \citep{botorch}, thus supporting both nonparametric and non-Gaussian surrogates. 
...
and the references therein. We take a moment to clarify the scope of our present setting in relation 
to a thread of research in the computer experiments literature.  
...
It is difficult to make broad assertions about specific emulator models, as the adequacy of a specific
model will depend on characteristics of $\fwd$ and the particular goals at hand. However, one common 
theme that drives certain model choices is the characteristics and dimensionality of the observation space;
i.e., the output space of $\fwd(\Par)$. If the output dimension $\dimObs$ is reasonably small, 
then a common approach is to fit an independent emulator to fit each output. In the GP setting, this would 
entail specifying a prior of the form
\begin{equation}
\fwdPrior \sim \GP(\emMeanPrior{\fwd}, \emKerPrior{\fwd}),
\end{equation}
where the prior mean $\emMeanPrior$ and kernel $\emKerPrior$ are of the independent multi-output variety.


multi-output GP prior directly for the forward model. 
...
This follows from general facts about Gaussians; see \Cref{prop:Gaussian_marginal_moments} in the appendix for a full derivation.
