...
\author{Andrew G. Roberts, Michael Dietze, Jonathan Huggins}
...
% Surrogate targets
\section{Choosing the Target Map}
...
\begin{figure}
     ...
     \begin{subfigure}[b]{0.3\textwidth} % GP dist [fwd]
         \centering
         \includegraphics[width=\textwidth]{gp_dist_fwdem.png}
         \caption{\texorpdfstring{$\targetEm$}{Forward Model Emulator}}
         \label{fig:gp_dist_fwdem}
     \end{subfigure}
     \hfill
     \begin{subfigure}[b]{0.3\textwidth} % llik dist [fwd]
         \centering
         \includegraphics[width=\textwidth]{llik_dist_fwdem.png}
         \caption{\texorpdfstring{$\log \lik(\cdot; \targetEm)$}{Induced Log-Likelihood Emulator}}
         \label{fig:llik_dist_fwd}
     \end{subfigure}
     \hfill
     \begin{subfigure}[b]{0.3\textwidth} % lik dist [fwd]
         \centering
         \includegraphics[width=\textwidth]{lik_dist_fwdem.png}
         \caption{\texorpdfstring{$\lik(\cdot; \targetEm)$}{Induced Likelihood Emulator}}
         \label{fig:lik_dist_fwdem}
     \end{subfigure} % Empty subfigure
    ...
    \end{subfigure}
     ...
     \begin{subfigure}[b]{0.3\textwidth} % llik dist [llik] 
         \centering
         \includegraphics[width=\textwidth]{llik_dist_llikem.png}
         \caption{\texorpdfstring{$\targetEm$}{Log-Likelihood Emulator}}
         \label{fig:llik_dist_llikem}
     \end{subfigure}
     \hfill
     \begin{subfigure}[b]{0.3\textwidth} % lik dist [llik]
         \centering
         \includegraphics[width=\textwidth]{lik_dist_llikem.png}
         \caption{\texorpdfstring{$\Exp{\targetEm}$}{Induced Likelihood Emulator}}
         \label{fig:lik_dist_llikem}
     \end{subfigure}
        \caption{Comparison of forward model and log-likelihood emulation in a simple one-dimensional problem 
        \texorpdfstring{($\dimPar = \dimObs = 1$)}{}. The top and bottom rows correspond 
        to the forward model emulation and log-likelihood emulation settings, respectively. The columns correspond to (1) the forward model emulator;
...
\begin{align}
\postDens(\Par; \targetEm) &= \priorDens(\Par)\Gaussian(\obs \given \targetEm(\Par), \likPar) \label{eq:fwd-em-Gaussian-post} \\
\targetEm(\Par) &\sim \Gaussian(\emMean(\Par), \emKer(\Par))
...
&= \priorDens(\Par) \Gaussian(\obs \given \emMean(\Par), \likPar + \emKer(\Par)) \\
\Var_{\Ndesign}\left[\postDens(\Par; \targetEm) \right]
&= \priorDens^2(\Par) \bigg[\frac{\Gaussian\left(\obs \given \emMean(\Par), \frac{1}{2}\likPar + 
\emKer(\Par)  \right)}{\det(2\pi \likPar)^{1/2}} - \\
&\qquad \qquad\qquad \frac{\Gaussian\left(\obs \given \emMean(\Par), \frac{1}{2}\left[\likPar + \emKer(\Par)\right]  \right)}{\det(2\pi [\likPar + \emKer[\Ndesign]{\fwd}(\Par)])^{1/2}}\bigg]
\end{align*}
...
$\targetEm[0] \sim \GP(\emMean[0], \emKer[0])$ for $\fwd$. Conditioning on the design yields 
the predictive distribution 
$\targetEm \Def \targetEm[0] \given [\targetEm[0](\design) = \fwd(\design)] \sim \GP(\emMean, \emKer)$, 
...
$\emMean[0], \emKer[0]$ as multi-output mean and covariance functions, respectively
