...
In fields ranging from climate science \citep{ESM_modeling_2pt0,FATES_CES} to systems biology 
\citep{yeastMatingSurrogate}, the scientific process increasingly relies on complex, high-fidelity 
computer simulations. Calibrating these models—estimating parameters consistent with observed data—naturally 
falls within the Bayesian paradigm. However, standard posterior inference algorithms (e.g., Markov chain Monte Carlo) 
require evaluating the likelihood function tens of thousands of times. When a single simulation takes minutes or hours, 
this cost renders these standard approaches intractable.
...
Other authors have acknowledged the importance of this choice, prior to committing to one of the 
two options \citet{Surer2023sequential,trainDynamics,ActiveLearningMCMC,emPostDens}.
\todo{Is this last sentence needed?}
...
\item \emph{Specialized Architectures}: Utilizing emulators designed to leverage the structure of particular
dynamical models 
...
the unnormalized log-posterior—collectively referred to as \inlinedef{log-density emulation}.
...
non-negativity of the resulting density approximation, and yield a smoother target surface.
...
However, this simplification introduces distinct challenges. Log-densities often exhibit a large dynamic 
...
to emulator misspecification \citep{ourSurrPropPaper}.

A further complication arises when likelihood parameters (e.g., the noise covariance $\likPar$ in \Cref{eq:inv_prob_Gaussian}) are unknown. 
While one can expand the emulator's input space to include these parameters \citep{llikRBF, emPostDens}, this increases 
the input dimensionality and can further complicate the target surface. Alternatively, some likelihoods permit emulating a 
sufficient statistic—such as the mean squared error—which can be evaluated independently of the noise 
parameters \citep{FerEmulation}.
...
Since the prior density is typically a known, analytic function, the log-likelihood is the most 
frequent target for emulation. However, several works target the unnormalized log-posterior 
...
via a deterministic shift by the log-prior---there are distinct modeling trade-offs.

Emulating the log-posterior allows the surrogate to leverage known tail behavior, as the posterior 
must decay to zero in the limit. In contrast, the log-likelihood's tail behavior may be poorly defined 
or non-existent (e.g., in the case of flat likelihoods), presenting a greater challenge for 
standard surrogate models (e.g., stationary GPs). Conversely, a downside of log-posterior emulation 
is that it ``bakes in'' a specific prior; any update to the prior model necessitates re-training the surrogate, 
whereas a log-likelihood emulator remains valid regardless of the prior choice.
In data-rich settings where the likelihood dominates the prior, the practical difference between these two 
targets diminishes, as the response surfaces will be nearly identical in the regions of high posterior mass.
...
\begin{figure}[p] % [p] suggests putting it on its own page
    ...
    \begin{subfigure}[t]{0.48\textwidth}
        \centering
        \includegraphics[height=0.22\textheight]{post_norm_dist_fwdem.png}
	\caption{\texorpdfstring{$\Gaussian(\obs \given \targetEm(\cdot), \likPar) / \normCst(\targetEm)$}{Induced Density Approximation}}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{0.48\textwidth}
        \centering
        \includegraphics[height=0.22\textheight]{post_norm_dist_ldensem.png}
...
\begin{figure}[t]
    \ContinuedFloat
    \caption{Pushforward distributions induced by underlying GP emulators of the forward model (left column) and unnormalized log-posterior (right column). 
    The plots summarize the pointwise marginal distributions of each quantity; in particular, the means (magenta lines)
    and 95\% credible intervals (shaded regions). The black lines are ground truth (no emulation) and the gray dashed lines indicate the locations of the 
    design inputs used to train the surrogates. The respective rows represent the surrogate-induced distributions over the 
    \emph{(1)} forward model, \emph{(2)} unnormalized log-posterior density, \emph{(3)} unnormalized posterior density, and 
    \emph{(4)} normalized posterior density. The top right entry is blank due to the fact that the log-density surrogate does not produce an approximation
    of the forward model. The magenta lines in the third and fourth rows represent \texorpdfstring{$\postApproxEUP$}{the unnormalized EUP}
    and \texorpdfstring{$\postApproxEP$}{the EP}, respectively.}
...
$\postDensNorm$ for downstream decision-making. Consequently, the central challenge in 
surrogate-based Bayesian inference is not merely fitting the emulator, but effectively utilizing it to 
approximate $\postDensNorm$ \citep{StuartTeck1,SinsbeckNowak}. Any discrepancy between 
the emulator and the target map introduces bias into the posterior estimate. 
While iterative refinement can asymptotically eliminate this bias (\Cref{sec:active-learning}), 
computational budgets often preclude this luxury.

To mitigate bias and properly calibrate uncertainty, one must acknowledge the surrogate's predictive 
uncertainty within the posterior approximation \citep{BilionisBayesSurrogates,BurknerSurrogate}. 
However, as noted by \citet{BurknerSurrogate}, there is no single ``correct'' mechanism for propagating 
...
this approach assumes the emulator is highly accurate, an assumption rarely satisfied in complex problems. 
In general, disregarding the surrogate uncertainty leads to posterior 
approximations that are both biased and overconfident \citep{BurknerSurrogate,BilionisBayesSurrogates,StuartTeck1}.

\subsection{Frameworks for Constructing Posterior Estimators}
To move beyond the plug-in baseline, we outline four conceptual frameworks that rigorously ground 
the propagation of surrogate uncertainty.
...
To circumvent the difficulties of a random normalizing constant, one can estimate the \textit{unnormalized} 
density $\postEm$ directly. This approach is computationally attractive as $\postEm(\Par)$ depends only 
on the marginal distribution of $\targetEm(\Par)$. Adopting a decision-theoretic stance \citep{SinsbeckNowak}, 
we seek the estimator $\qDensOpt$ that minimizes the expected loss
\begin{equation}
...
correlation structure of the surrogate \citep{VehtariParallelGP}. This decision theoretic approach was originally proposed in 
...
Generalized Bayesian Inference \citep{Bissiri2016,knoblauch2019} by specifying a loss function 
$\gibbsloss(\Par; \targetEm)$. Viewing this loss as a random function of $\targetEm$, let $\risk$ 
...
\footnote{Let $\KL{p}{q}$ denote the Kullback-Leibler (KL) divergence between two densities.}
...
\end{align}
The learning rate $\gibbsrate$ governs the weight of the data relative to the prior. This framework is particularly useful 
for robustifying inference against surrogate misspecification; by tuning $\gibbsrate$, one can temper the posterior to 
account for epistemic errors in the emulator \citep{Bissiri2016}. Notably, with $\gibbsrate = 1$ and particular definitions 
...
\label{eq:ep}
\end{align}
\citet{ourSurrPropPaper} derives the EP from this perspective and argues for its use as a principled default posterior approximation in many applications.
Functionally, the EP is a mixture distribution averaging the posteriors induced by all possible surrogate trajectories. 
This interpretation suggests a nested sampling scheme (\Cref{alg:ep}), marginalizing over surrogate trajectories $\targetTraj \sim \emDist$.
...
Practical implementation typically employs Metropolis-within-Monte-Carlo (MwMC), in which a separate MCMC scheme is
run for each iteration of the loop (line 2) in order to produce approximate samples $\Par^{(\sampleIndex, m)}$
...
approximate schemes tailored for GP emulators to overcome this bottleneck \citep{ourSurrPropPaper,trainDynamics}.
...
\label{eq:eup}
\end{align}
The EUP corresponds to the $L_2$-optimal estimator under the unnormalized decision-theoretic framework 
(\Cref{sec:decision-theory}; \citet{SinsbeckNowak}). Because the expectation $\emE[\postEm(\Par)]$ can often 
...
\begin{figure}
     ...
     \begin{subfigure}[b]{0.48\textwidth} % lpost dist [fwd]
         ...
         \caption{\texorpdfstring{$\log \Gaussian(\obs \given \targetEm(\cdot), \likPar)$}{Log Post. Approx., Forward Model Em.}}
     ...
     \begin{subfigure}[b]{0.48\textwidth} % lpost dist [ldens]
         ...
         \caption{Log-Posterior Emulator}
...
The instability of the EUP in log-density emulation (\Cref{ex:eup-ldens-em}) has motivated alternative pointwise statistics.
\begin{itemize}
    ...
    regions with high surrogate variance, encouraging the posterior to decay away from design points. While this encourages 
    tail decay in the posterior estimate, it neglects uncertain regions that may in reality contain posterior mass.
...
In trivial settings, a static design sampled from the prior may suffice to train an accurate surrogate. 
However, for complex inverse problems, the posterior distribution often concentrates on a small subset of the prior support. 
A ``one-shot'' design---while allowing for maximal parallelism---allocates computational resources inefficiently, 
placing the vast majority of design points in regions of negligible posterior mass. Conversely, the limiting case of 
a \inlinedef{pure sequential} strategy, where simulations are performed serially, 
maximizes information gain per run but fails to exploit parallel computing resources.

To balance these extremes, practical approaches typically adopt a batch-sequential workflow: simulator runs are 
spread across multiple rounds, with earlier results guiding the selection of future batches. We refer to this broad 
class of algorithms as \inlinedef{active learning} for Bayesian inverse problems. The central challenge in this 
regime is navigating the ``exploration vs. exploitation'' trade-off \citep{reviewBayesOpt,BadiaRL}. Active learning 
algorithms must exploit current knowledge to refine the surrogate in known high-probability regions, while 
simultaneously exploring high-uncertainty areas to avoid missing other important regions. We categorize sequential 
strategies into three (often overlapping) paradigms:
\begin{enumerate}
...
In this framework, new design points are chosen by optimizing a criterion, or \inlinedef{acquisition function}, that quantifies the 
utility of running the simulator at a set of candidate locations. Similar approaches are widely applied in the broader context
of surrogate modeling. For example, the field of Bayesian optimization focuses on the definition of acquisition functions 
targeting the goal of black-box function optimization \citep{reviewBayesOpt}. In the present context, acquisition functions
should reflect the explicit goal of estimating the posterior distribution. 

A subtle but critical consideration in this setting is the optimal distribution of design points. Even with a perfect surrogate, 
approximating a probability distribution requires placing more points in regions of high probability mass. In fact, theoretical 
results suggest that design points should be sampled from a distribution that is \textit{over-dispersed} relative to the target 
posterior, thereby ensuring adequate coverage of the tails \citep{sinsbeck2020sequential}. Design criteria must therefore 
balance three objectives: (i) reducing surrogate uncertainty, (ii) focusing on high posterior mass, and (iii) ensuring tail coverage.

\subsubsection{Frameworks for Design Criteria}
Similar to the posterior estimators in \Cref{sec:target-map}, design criteria can be derived from rigorous theoretical frameworks.

\begin{comment}
\paragraph{Decision Theoretic Framework.}
Following the perspective in \Cref{sec:decision-theory}, one can select the design that minimizes the expected loss of the resulting posterior estimator. For an unnormalized density estimator $\qDens$, we seek the design $\batchPar$ that minimizes the expected future risk:
\begin{equation}
    \alpha(\batchPar) = \emE_{\batchObs}\left[ \min_{\qDens} \emE_{\targetEm | \batchObs} [\loss(\qDens, \postEm)] \right],
\end{equation}
where the outer expectation averages over potential simulation outcomes at $\batchPar$. This effectively anticipates the reduction in posterior approximation error provided by the new data.

\paragraph{Stepwise Uncertainty Reduction (SUR).}
Alternatively, SUR strategies seek to minimize an information-theoretic summary of the uncertainty in the quantity of interest \citep{bect2019supermartingale,chevalier2014fast}. If the quantity of interest is the posterior density, this amounts to selecting $\batchPar$ to minimize the expected entropy or integrated variance of the future posterior approximation.

\subsubsection{Pointwise Criteria}
Pointwise criteria assign a utility score to single candidate points $\Par \in \parSpace$. Common examples include Maximum Variance (MaxVar) and Maximum Entropy (MaxEnt), which target the location of highest surrogate uncertainty. While nominally designed for pure sequential settings, these can be adapted for batch acquisition using heuristic strategies. The \inlinedef{Kriging Believer} (or Constant Liar) heuristic \citep{ginsbourger2010kriging} selects a point, temporarily updates the surrogate assuming a deterministic outcome (e.g., the predictive mean), and repeats the process until the batch is filled. This prevents the "clumping" of design points that occurs if one simply selects the top-$k$ maxima of the acquisition function.

\subsubsection{Global Criteria}
Global criteria assess uncertainty integrated over the entire parameter space. These are naturally suited for batch selection, as the reduction in integrated uncertainty can be conditioned on a set of candidate points simultaneously.
Crucially, the "goal" of the inference can be encoded into the integration measure. A generic global criterion takes the form:
\begin{equation}
    \alpha(\batchPar) = \int_{\parSpace} \mathbb{V}[\qDens(\Par) \mid \batchPar] \, w(\Par) \, \d\Par.
\end{equation}
Here, $\mathbb{V}$ represents the variance of the estimator conditioned on the proposed batch. The weight function $w(\Par)$ dictates the focus:
\begin{itemize}
    \item \textbf{Prior Weighting ($w = \priorDens$):} Reduces uncertainty globally according to the prior support. This is robust but may waste effort on low-likelihood regions.
    \item \textbf{Posterior Weighting ($w \approx \postDens$):} Weights the integral by the current posterior approximation. This focuses the design on the posterior mode, but risks getting stuck in local optima if the current approximation is biased.
\end{itemize}
Alternatively, one can integrate the variance of the \textit{forward model} $\target$, weighted by the posterior probability. This targets regions where the map is uncertain \textit{and} the probability is high, explicitly coupling the surrogate accuracy to the inference task \citep{sinsbeck2020sequential, activeSubspaceLearning}.
...
\subsection{Amortized and Simulation-Based Inference}
