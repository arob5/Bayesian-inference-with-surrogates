...
\citet{gramacy2020surrogates,gpML}.

The typical workflow consists of specifying a GP prior $\targetEm[0] \sim \GP(\emMean[0], \emKer[0])$ 
over the target function $\target$, with $\emMean(\cdot)$ and $\emKer(\cdot, \cdot)$ denoting the prior 
mean and covariance functions, respectively. The 
defining property of a GP is that each of the finite-dimensional distributions are Gaussian 
