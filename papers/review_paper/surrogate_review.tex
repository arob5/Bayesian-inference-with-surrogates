...
\begin{figure}
     ...
     \begin{subfigure}[b]{0.48\textwidth} % lpost dist [fwd]
         \centering
         \includegraphics[width=\textwidth]{lpost_dist_fwdem_trim.png}
     ...
     \begin{subfigure}[b]{0.48\textwidth} % post norm approx [fwd]
         \centering
         \includegraphics[width=\textwidth]{post_norm_approx_fwdem.png}
     ...
     \begin{subfigure}[b]{0.48\textwidth} % lpost dist [llik]
         \centering
         \includegraphics[width=\textwidth]{lpost_dist_llikem_trim.png}
     ...
     \begin{subfigure}[b]{0.48\textwidth} % post norm approx [llik] 
         \centering
         \includegraphics[width=\textwidth]{post_norm_approx_llikem.png}
         \caption{$\postDensNorm$ approx}
...
\subsection{Estimators for the Random Posterior}
We now consider surrogate-based posterior approximations that incorporate the surrogate uncertainty.
Recall the shorthand $\postNormEm(\Par) = \postEm(\Par) / \normCstEm$ for the random quantities induced
by plugging $\targetEm$ in place of $\target$. One approach to deriving posterior approximations is to construct
a deterministic estimate that summarizes the uncertainty in $\postNormEm$. This perspective is 
adopted in \citep{ourSurrPropPaper,StuartTeck1,StuartTeck2,random_fwd_models,TeckHyperpar}.
...
showing that it represents the optimal estimate of $\postNormEm$ with respect to several 
common loss functions. The terminology \textit{expected posterior} comes from 
...
of the EP \citep{StuartTeck1,StuartTeck2,SinsbeckNowak}. \citet{ourSurrPropPaper} discuss practical computation for the EP, presenting
an approximate MCMC scheme that is well-defined in the GP setting. This avoids the MwMC requirement of running a large
ensemble of MCMC chains. \citet{trainDynamics} alternatively suggest an approximate MwMC implementation geared towards
GP emulators.

\subsubsection{The Expected Unnormalized Posterior}
The EP averages the \textit{normalized} random posterior $\postEm(\Par) / \normCstEm$, 
acknowledging the coupling between the numerator and denominator induced by the 
shared dependence on $\targetEm$.
Since the normalizing constant is the root cause of the challenges associated with EP 
computation, many studies have opted to circumvent these difficulties by first deriving 
an estimate of the \textit{unnormalized} density approximation $\postEm$ and then 
normalizing post-hoc. This is computationally convenient since
 $\postEm(\Par)$, unlike $\postNormEm(\Par)$, depends only on the single point 
 estimate $\targetEm(\Par)$ rather than the entire surrogate $\targetEm$.
Thus, a consequence of using such \inlinedef{pointwise estimators} is that they ignore any 
predictive correlation structure in the surrogate. 

The most common posterior estimate of this sort is the \inlinedef{expected unnormalized posterior (EUP)}
\footnote{
...
\begin{align}
\postApproxEUP(\Par) \Def \frac{\emE[\postEm(\Par)]}{\emE[\normCstEm]},
\label{eq:eup}
\end{align}
which averages the numerator and denominator independently. The EUP can also 
be interpreted as the marginal posterior $p(\Par \given \obs)$ under the hierarchical model
\begin{equation}
...
\end{equation}
as discussed in \citet{ourSurrPropPaper,SinsbeckNowak,StuartTeck1,StuartTeck2}.
The papers \citet{StuartTeck1,StuartTeck2,random_fwd_models} establish bounds between the EUP
and ground truth posterior. \citet{ourSurrPropPaper} studies the EUP as an approximation
...
\begin{examplebox}{EUP, Gaussian Forward Model Emulator Setting}{eup-fwd-em}
Consider the setting from \Cref{ex:fwd-em}, where a Gaussian forward model surrogate is paired with 
an additive Gaussian likelihood. In this case, the EUP is given by
\begin{align*}
\postApproxEUPNorm(\Par)
\propto \emE\left[\priorDens(\Par)\Gaussian(\obs \given \targetEm(\Par), \likPar) \right] 
&= \priorDens(\Par) \Gaussian(\obs \given \targetEm(\Par), \likPar + \emVar(\Par)).
\end{align*}
\end{examplebox}

\begin{examplebox}{EUP, Gaussian Log-Likelihood Emulator Setting}{eup-ldens-em}
In the setting from \Cref{ex:ldens-em} with a log-likelihood emulator with Gaussian predictive
distributions, the EUP is given by
\begin{align*}
\postApproxEUPNorm(\Par)
...
\end{align*}
Due to the exponential dependence, this distribution can be highly sensitive to small changes in
$\emMean(\Par)$ and $\emVar(\Par)$. In particular, the EUP in this setting is susceptible to 
...
a more robust alternative to the mean. \citet{gpEmMCMC} consider a weighted quantile variant that 
results in an estimator that penalizes surrogate uncertainty. This generally has the effect of enforcing
tail decay in the estimator as distance from the design points increases, but fails to produce an
uncertainty-aware posterior approximation. \Cref{ex:other-pw-ldens-em} details the particular
form of these estimators for surrogate models with Gaussian predictions.
...
{\parbox{0.85\linewidth}{%
    Other Pointwise Estimators\\
    Gaussian Log-Likelihood Emulator Setting}
}
{other-pw-ldens-em}
We return to the setting from \Cref{ex:ldens-em} with a log-likelihood emulator with Gaussian predictive
distributions. Let $\emQ$ denote the $\quantileProb \in (0, 1)$ quantile of a random variable with respect to the
emulator predictive distribution. A quantile-based estimate in this setting takes the form
\begin{align*}
...
the density will be inflated in regions of higher surrogate uncertainty.
\end{examplebox}
