...
\begin{examplebox}{Forward Model Surrogate, Gaussian Setting}{ex:fwd-em}
...
\begin{examplebox}{Log-Likelihood Surrogate, Gaussian Setting}{ex:ldens-em}
...
\begin{align*}
\emE\left[\postDens(\Par; \targetEm) \right] 
&= \Exp{\emMean(\Par) + \frac{1}{2} \emVar(\Par)} \\
\Var_{\emDist}\left[\postDens(\Par; \targetEm) \right]
&= \left[\Exp{\emVar(\Par)} - 1 \right] \Exp{2\emMean(\Par) + \emVar(\Par)}
\end{align*}
\end{examplebox}
