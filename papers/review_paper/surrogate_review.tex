...
aiming to approximate the posterior distribution using only a small set of density evaluations. Many such approaches 
...
use ``emulator'' as a synonym, with ``emulate'' referring to the process of constructing an emulator. 
\end{remark}
...
The implicit assumption here is that the target map has been chosen to alleviate the computational bottleneck; that is,
...
In practice, fitting a highly accurate surrogate can be challenging. The induced posterior approximation inherits the 
imperfections from the underlying emulator.
