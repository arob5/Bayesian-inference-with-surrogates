...
being to select a new batch $\ParBatch \subset \parSpace$ of $\Nbatch$ design points. Denoting the acquisition function
by $\acq: \parSpace^\Nbatch \to \R$, the design selection problem is formalized as 
\begin{equation}
...
an unnormalized posterior estimate is chosen as the optimizer of an expected loss $\emE[\loss(\qDens, \postEm)]$
over candidate functions $\qDens$. The value of this objective function evaluated at the optimizer,
denoted $\qDens_\Ndesign$, is known as the \textit{Bayes' risk}. A natural design criterion is thus to select the 
batch $\ParBatch$ that would yield an updated optimum with the lowest possible Bayes' risk:
\begin{equation}
\acq(\ParBatch) \Def \emEAug[\loss(\qDens_{\Naugment}, \postEm[\Naugment])]
\end{equation}

In principle, one could attempt to develop 
...
\begin{equation}
    \alpha(\ParBatch) = \emE[\Naugment] \left[ \min_{\qDens} \emE_{\targetEm | \batchObs} [\loss(\qDens, \postEm)] \right],
\end{equation}
...
\begin{equation}
\acq(\ParBatch) \Def 
\int_{\parSpace} \E_{\gamma} \left\{\Var_{\Naugment}[\postDens(\Par; \targetEm[\Naugment]^{\ParBatch,\gamma}) \given \gamma]\right\} \ \rho(\Par) d\Par,
 \label{eq:acq-intvar}
\end{equation}
