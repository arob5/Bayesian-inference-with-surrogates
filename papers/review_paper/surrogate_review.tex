...
\input{../methods_paper/macros.tex}
...
\end{equation}
Observe that the likelihood evaluation $\lik(\Par; \obs)$ depends on the corresponding forward model evaluation $\fwd(\Par)$.
This latter computation may involve running an expensive computer simulation (e.g., a numerical differential equation solver)
...
The Bayesian approach completes the model specification with a prior distribution 
...
\end{align}
We emphasize that $\postDens(\Par)$ denotes the \textit{unnormalized} 
...
\begin{align}
\llik(\Par; \fwd) &= \log\Gaussian(\obs \given \fwd(\Par), \likPar) \\
...
algorithm. Such algorithms are serial in nature, often requiring $10^5 - 10^7$ iterations, with each 
...
evaluations $\fwd(\Par)$ incur significant computational cost. This problem has motivated the
development of numerous computationally cheap approximations to either 
$\fwd(\Par)$ or $\lpost(\Par)$ (note that approximating the former induces 
an approximation of the latter). We focus on surrogates that take the form of a  
...
The problem of estimating the parameters of differential equation models 
 can be cast within the Bayesian inverse problem framework. One motivating source of applications 
 stems from the use of dynamical models in Earth system modeling  
...
\end{align}
for some ``true'' parameter value $\ParTrue \in \parSpace$. Observe that this setting falls within the generic 
inverse problem framework with forward model $\fwd \Def \obsOp \circ \solutionOp$. 
Generalizations can be considered to cases with non-additive or non-Gaussian noise, or the 
presence of model discrepancy.
