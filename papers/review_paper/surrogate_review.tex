...
examples, both the Bayesian decision theoretic and the SUR frameworks produce acquisitions of the
ECU variety. The expected variance term in \Cref{eq:acq-intvar} admits a closed form in the settings of 
...
would yield a stochastic optimization problem. A simpler and more common solution is 
...
\end{align}
in which the $\Par_j$ are sampled once and then fixed, yielding the 

This is especially the case in our present setting,
...
Uncertainty in acceptance probability, sampling-based approaches (Murray SBI paper fixed-point iteration, Istem, etc.)
