...
imperfections from the underlying emulator. This implies the concern that the surrogate-based posterior approximation
...
\begin{remark}
Conceptually, we typically think of the randomness in the predictive distribution as quantifying \textbf{epistemic uncertainty}, which in principle could be reduced, were it computationally feasible to evaluate 
...
represent the true target function. \citet{BurknerSurrogate} present a framework that incorporates both
...
the $\Nbatch \times \Nbatch^\prime$ cross covariance.

Of central importance to this work is the manner in which emulator uncertainty propagates to downstream 
quantities of interest. For example, plugging the random surrogate in place of the true target map induces a distribution over the posterior density $\postDensNorm(\Par; \targetEm)$. 
...
model. In this case, $\emMean(\Par)$ and $\emVar(\Par)$ denote the predictive mean vector and 
covariance matrix, respectively, over the different outputs. To avoid complicating notation, we do not extend
the above vectorized notation in the multi-output setting. We make clarifying remarks when it is necessary to 
avoid ambiguity.

\subsection{Common Surrogate Models}
In this section, we highlight several popular surrogate models that fall 
