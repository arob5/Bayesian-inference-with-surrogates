...
VillaniAdaptiveGP,weightedIVAR,idealizedGCM,CES}. \Cref{fig:em_dist_1d}
...
The primary appeal of this approach is \textit{scalarization} \citep{ranjan2016inverse, trainDynamics}: 
...
A further complication arises when likelihood parameters (e.g., $\likPar$ in \Cref{eq:inv_prob_Gaussian}) are unknown. 
...
ecological modeling are considered in \citet{FerEmulation,FATES_CES}, while \citep{GP_PDE_priors}
...
While the choice may seem numerically inconsequential---as one can be converted to the other 
...
surrogate modeling is not merely fitting the emulator, but effectively utilizing it to 
...
this approach assumes the emulator is highly accurate. In general, disregarding the surrogate uncertainty leads to posterior 
...
Given that the emulator $\targetEm$ is a random function, the induced posterior $\postNormEm(\Par)$ is a random density
...
random function $\targetEm$---often renders inference challenging for densities of this form.
...
Practical implementation typically employs Metropolis-within-Monte-Carlo (MwMC), in which approximate samples 
$\Par^{(\sampleIndex, m)}$ are generated by an MCMC scheme targeting the posterior trajectory 
$\postDensNorm(\cdot; \targetTraj^{(\sampleIndex)})$ \citep{garegnani2021NoisyMCMC, BurknerSurrogate}.
...
approximate schemes tailored for GP emulators to surmount this bottleneck \citep{trainDynamics,ourSurrPropPaper}.
\citet{ourSurrPropPaper} argues for the EP as a principled default posterior choice in many applications.
