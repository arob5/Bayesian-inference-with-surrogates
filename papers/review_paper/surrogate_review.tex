...
\subsection{Tempered Approximations} \label{sec:tempering}
...
between the prior and posterior. Since the earlier distributions will be less concentrated, the surrogate 
...
While tempering methods are diverse, we restrict our focus to two broad categories:
(1) SMC methods primarily relying on importance sampling techniques; and (2) algorithms that 
utilize approximate Kalman-style updates to traverse between the bridging distributions. 

\paragraph{SMC Methods.}
\citet{BilionisBayesSurrogates} utilize SMC to characterize the EUP approximation under a GP 
forward model emulator, but do not actively adapt the surrogate design. \citet{quantileApprox} 
leverage SMC with a GP log-posterior surrogate, augmenting the design by sampling new 
points from the current posterior approximation at each iteration. \citet{activeReducedTempering} 
propose an adaptive SMC scheme that actively samples new design points to reduce surrogate error, 
in addition to using an entropy-based criterion to adapt the temperature schedule $\{\beta_t\}$.

\paragraph{Kalman Methods.}
Calibrate, Emulate, Sample (CES)



- Youssef work, Joseph work
- EKI type methods
- SIS type methods (like paper I read on plane, quantile estimator, Bilionis)
