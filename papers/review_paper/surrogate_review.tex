...
quantify parameter uncertainty and balance prior information with observed data \citep{KOH}.
...
While conceptually straightforward, replacing the true model with an approximation introduces many methodological
and conceptual challenges. Invariably, the surrogate is an imperfect representation of the true
model. Posterior estimates derived using the surrogate will inherit these imperfections, prompting the 
concern of biased and overconfident inference.
...
model classes. \Cref{sec:target-map} discusses the decision as to which quantity to actually 
emulate, defining the distinction between forward model and log-density emulators. 
...
posterior approximations that properly incorporate emulator uncertainty, while the latter
reviews active learning algorithms that sequentially build up surrogate models.  

\subsection{Challenges in Surrogate-Based Bayesian Inference}

While some of these difficulties are generic within the broader context of surrogate modeling,
