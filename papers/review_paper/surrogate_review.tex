...
eliminating the expectation over $\targetEm(\ParBatch)$. In principle, this framework could be modified to consider a loss
...
\begin{equation}
\acq^{\mathrm{maxvar}}(\Par) \Def -\Var_{\emDist}[\postEm(\Par)], \label{eq:acq-maxvar}
\end{equation}
...
{maxvar-ldens}
Consider the Gaussian log-density emulation setting from \Cref{ex:eup-ldens-em}
where $\postEm(\Par) = \priorDens(\Par) \Exp{\targetEm(\Par)}$, $\targetEm \sim \GP(\emMean, \emKer)$.
In this case, the maximum variance acquisition is
\begin{equation}
\acq^{\mathrm{maxvar}}(\Par) = -\priorDens^2(\Par) \left[\Exp{\emVar(\Par)} - 1 \right] \Exp{2\emMean(\Par) + \emVar(\Par)},
\end{equation}
...
points with the largest acquisition values can result in a batch of very similar points. Heuristics 
...
\footnote{As before, this could be extended to operate directly on the normalized densities. While arguably more correct,
...
\begin{equation}
\acq^{\mathrm{evar}}(\ParBatch) \Def 
\int_{\parSpace} 
\E_{\gamma} \left\{\Var_{\emDist[\Naugment]}[\postDens(\Par; \targetEm[\Naugment]^{\ParBatch,\gamma}) \given \ParBatch, \gamma]\right\} \ \weightdens(\Par) \d\Par,
 \label{eq:acq-intvar}
\end{equation}
where $\weightdens$ is a density over $\parSpace$. ECU criteria naturally accommodate batch acquisitions since 
$\ParBatch$ enters as a conditioning variable. Variations of ECU criteria can be created by changing the measure 
of uncertainty (e.g., variance, entropy, interquartile range), the target quantity (e.g., $\postEm$, $\funcEm$), and the 
weighting measure $\weightdens$. For example, targeting the variance of $\funcEm$ with uniform weighting measure 
yields the classical integrated mean squared prediction error \citep{Mercer_kernels_IVAR}. As illustrated in the below
examples, both the Bayesian decision theoretic and the SUR frameworks produce produce acquisitions of the
ECU variety.
...
\todo[inline]{connection to decision theory and SUR}

This acquisition is considered in \citet{VehtariParallelGP}, but the authors ultimately recommend
alternative robust criteria based on the interquartile range in place of the variance.
\end{examplebox}
...
{evar-ldens}
\begin{align}
\acq(\ParBatch) &= \frac{1}{J}
\sum_{j=1}^{J} \priorDens^2(\Par_j) \bigg[\frac{\Gaussian\left(\obs \given \emMean(\Par_j), \CovComb(\Par_j) - \frac{1}{2}\likPar \right)}{2^{\dimObs/2} \det(2\pi \likPar)^{1/2}} - \label{eq:acq-intvar-saa-fwd} \\
&\qquad\qquad \frac{\Gaussian\left(\obs \given \emMean(\Par_j), \CovComb(\Par_j) - \frac{1}{2}\CovComb[\Naugment](\Par_j) \right)}{2^{\dimObs/2} \det(2\pi \CovComb[\Naugment](\Par_j))^{1/2}} \bigg], \nonumber
\end{align}
where $\CovComb(\Par) \Def \likPar + \emKer(\Par)$ and $\CovComb[\Naugment](\Par) \Def \likPar + \emKer[\Naugment](\Par)$.  
...
\sum_{j=1}^{J} \emKer[\Naugment(\Par_j),
...
is chosen to target the goal of posterior approximation. This has the 
benefit of being much easier to compute, since the GP variance $\emKer[\Naugment](\Par)$ is independent of the 
