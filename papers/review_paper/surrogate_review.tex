...
\todo[inline]{bring up optimization perspective here?} 
...
Let $\risk$ denote a risk functional mapping $\gibbsloss(\Par, \targetEm)$ to a deterministic summary
$\risk(\Par)$. A posterior approximation can then be defined as the optimizer
\begin{align}
\qDensNormOpt \Def 
\mathrm{arginf}_{\qDensNorm \in \qSpaceNorm} 
\left\{\int \emE[\gibbsloss_\Par(\targetEm; \obs)] \qDensNorm(\d\Par) + \frac{1}{\gibbsrate} \KL{\qDensNorm}{\priorDens} \right\},
\end{align}
over the space of densities $\qDensNorm$. The \textit{learning rate} $\gibbsrate > 0$ controls the size of the 
...
\todo[inline]{cite GP emulated MCMC, Fer, noisy MCMC literature}
...
prior reversion. The particular EUP estimator in \Cref{eq:eup-gaussian}
...
reversion under surrogate ignorance.
Due to the exponential dependence, this distribution can be highly sensitive to small changes in
$\emMean(\Par)$ and $\emVar(\Par)$, making it particularly susceptible to 
...
a more robust alternative to the mean. \Cref{ex:other-pw-ldens-em} details the particular form 
...
tail decay in the posterior estimate as distance form the design points increases, but but fails to produce 
