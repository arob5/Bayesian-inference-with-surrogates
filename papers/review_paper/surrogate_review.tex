...
In trivial settings, a static design sampled from the prior may suffice to train an accurate surrogate. However, for complex inverse problems, the posterior distribution often concentrates on a minute subset of the prior support. A "one-shot" design—while allowing for maximal parallelism—allocates computational resources inefficiently, placing the vast majority of design points in regions of negligible posterior mass. Conversely, the limiting case of a \inlinedef{pure sequential} strategy, where simulations are performed serially, maximizes information gain per run but fails to exploit parallel computing resources.

To balance these extremes, modern approaches typically adopt a batch-sequential workflow: simulator runs are spread across multiple rounds, with earlier results guiding the selection of future batches. We refer to this broad class of algorithms as \inlinedef{active learning} for Bayesian inverse problems. The central challenge in this regime is the "exploration vs. exploitation" trade-off \citep{reviewBayesOpt,BadiaRL}. The algorithm must exploit current knowledge to refine the surrogate in high-probability regions, while simultaneously exploring areas where surrogate uncertainty is high to avoid missing modes.

We categorize sequential strategies into three (often overlapping) paradigms:
\begin{enumerate}
    \item \inlinedef{Design Optimization}: Explicitly maximizing an acquisition function to select the next batch of points.
...
In this framework, new design points are chosen by optimizing a criterion, or \textit{acquisition function}, that quantifies the utility of observing the simulator at a candidate location. Unlike standard experimental design—which often targets global response surface accuracy—active learning for inversion must be "goal-oriented," focusing strictly on the accuracy of the resulting posterior approximation.

A subtle but critical consideration in this setting is the optimal distribution of design points. Even with a perfect surrogate, approximating a probability distribution requires placing more points in regions of high probability mass. In fact, theoretical results suggest that design points should be sampled from a distribution that is \textit{over-dispersed} relative to the target posterior, thereby ensuring adequate coverage of the tails \citep{sinsbeck2020sequential}. Design criteria must therefore balance three objectives: (i) reducing surrogate uncertainty, (ii) focusing on high posterior mass, and (iii) ensuring tail coverage.
