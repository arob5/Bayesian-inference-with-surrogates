...
Given computational costs, these new evaluation points should be carefully chosen in 
...
in several studies \citep{StuartTeck2, gp_surrogates_random_exploration,SinsbeckNowak,Surer2023sequential,adaptiveMultimodal}. 
We emphasize that traditional active learning algorithms targeting global improvements in the 
underlying emulators (e.g., $\fwdEm[\Ndesign]$ or $\lpostEm[\Ndesign]$) may be sub-optimal with 
respect to the goal of improving the induced posterior approximation.

Effective sequential design strategies target regions where the current surrogate 
expects high posterior mass, in addition to regions where the surrogate is uncertain.
...
VehtariParallelGP,VillaniAdaptiveGP}. We also highlight batch sequential approaches, 
where multiple design points are acquired simultaneously. Batch sequential design becomes 
increasingly essential as the cost of likelihood evaluations increases, as it allows for 
...
tradeoff. The maximum variance criterion is utilized in \citet{Lueckmann2018LikelihoodfreeIW,Kandasamy_2017}.
A similar approach is taken in \citet{wang2018adaptive}, but entropy is utilized as the measure of uncertainty
instead of variance. \citet{gp_surrogates_random_exploration} consider an alternative, where exploration and 
exploitation are explicitly decoupled by selecting one point that maximizes $\E_{\Ndesign}[\funcEm[\Ndesign](\Par)]$ 
and randomly sampling a second point from $\priorDens$. 
...
--integral
-batch design
-other approaches (MCMC-based, EKI)
