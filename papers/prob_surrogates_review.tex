...
We do not discuss these methods further
...
numerically-inaccessible function $\fwd$.
Plugging the random function $\fwdEm[\Ndesign]$ in place of $\fwd$ in the log-likelihood 
thus induces random approximations of the likelihood and posterior. In general, we use 
...
$\llik(\Par; \fwdEm[\Ndesign]) = \llikEm[\Ndesign](\Par)$ when it is necessary to specify that
the log-likelihood approximation is induced by a forward model emulator. We similarly use 
the notation $\postDens(\Par; \fwdEm[\Ndesign]) = \postEm(\Par)$ and 
$\lpost(\Par; \fwdEm[\Ndesign]) = \lpostEm(\Par)$ for the induced unnormalized 
...
temporal averages \citep{ESM_modeling_2pt0,idealizedGCM}.
...
been considered frequently in the literature \citep{VillaniAdaptiveGP}.
...
then the independent GP approach may be infeasible or ill-advised.   
...
The surrogate predictive distribution remains Gaussian under this model, and thus \Cref{fwd_em_lik_emulator_moments_Gaussian} still applies in this setting. The particular setup of a Gaussian likelihood with a GP forward model emulator using the basis vector approach is explored in \citet{Surer2023sequential}. 
...
result is the analog of \Cref{fwd_em_lik_emulator_moments_Gaussian} for log-density emulation.
...
Present the general marginal posterior (or expected posterior). Cite Burkner, and discuss how 
computation takes the form of an ensemble of MCMC runs. Then discuss how this runs into 
issues with nonparametric emulators. They also mention the use of the prior log density as a 
...
Consider mean approx: \citet{emPostDens}


\subsection{Existence and Integrability}
