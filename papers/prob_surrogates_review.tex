...
may serve various downstream goals. If computational constraints prevent further reduction of epistemic
uncertainty due to the high cost of augmenting the design, then one must accept that posterior inference 
will be approximate. In such cases, the surrogate uncertainty should be acknowledged to prevent 
...
\end{align}
that summarizes the uncertainty about the true posterior. 
...
\begin{equation}
\postNormEm[\Ndesign] \Def \E_{\Ndesign}\left[ \postEm[\Ndesign] \right], \label{eq:ep-approx}
\end{equation}
...
\begin{algorithm}
    ...
    \Function{sampleEP}{$\llikEmRdm[\Ndesign]{\postDensNorm}, \NSample$}     
        \For{$\sampleIndex \gets1,\NSample$} 
        		\State $\llikEmRdm[\Ndesign]{\postDensNorm}^{(\sampleIndex)} \sim \law(\llikEmRdm[\Ndesign]{\postDensNorm})$ \Comment{Sample a distribution}
...
previous literature instead proposes deterministic approximations of the \textit{unnormalized}
density. As opposed to the expected posterior method, these approximations are computed 
\textit{pointwise} in $\Par$; for GP surrogates, this implies that they do not incorporate the GP 
predictive covariance.
The practical benefit of this approach is that the unnormalized density approximation
can be input into standard Bayesian computation software.
In this section, we define multiple unnormalized density approximations that have been 
used in practice. Unlike in the previous section, the precise definition of these approximations
will depend on the underlying quantity being emulated; we present generic definitions for forward 
model $\fwdEm[\Ndesign]$ and log-likelihood $\llikEm[\Ndesign]$ emulators (the analogs 
for log-posterior emulators are similar to the log-likelihood case). 
We also provide closed-form unnormalized density approximations for the Gaussian settings defined in 
\cref{prop:fwd-em-Gaussian} and \cref{prop:llik-em-Gaussian}.
The definitions throughout this section are given for approximations to the \textit{unnormalized} posterior 
density, with the normalizing constants implicitly defined.

\begin{tabular}{lC{10cm}}
\toprule
\textbf{Posterior Approximation} & \textbf{Normalized Density} \\
\midrule
Plug-In Mean & 
\(\displaystyle \postDens(\Par; \E_{\Ndesign}\left[\fwdEm[\Ndesign](\Par)\right]) / \normCst(\E_{\Ndesign}[\fwdEm[\Ndesign]])  \) \\ 
Expected Likelihood & 
\(\displaystyle \E_{\Ndesign} \left[\postDens(\Par; \fwdEm[\Ndesign])\right] / \E_{\Ndesign}\left[\normCstEm[\Ndesign] \right] \) \\
Expected Posterior & 
\(\displaystyle \E_{\Ndesign}\left[\postDens(\Par; \fwdEm[\Ndesign]) / \normCstEm \right]  \) \\
\bottomrule
\end{tabular}
\caption{The approximations to the Metropolis-Hastings (MH) acceptance probability implied by different posterior approximations. All
expectations are with respect to the underlying surrogate predictive distribution. For brevity, the acceptance probabilities 
are presented for the case of a symmetric proposal distribution, but this assumption is not required.}
\label{tab:acc-prob-comparison}
\end{table}

\subsubsection{Expected Likelihood}
The expected posterior approximation considers the expectation of $\normCstEm[\Ndesign]^{-1} \postEm(\Par)$.
We could instead consider taking the expectation of $\postEm(\Par)$ and normalizing after-the-fact.
...
approximation, again borrowing terminology from
 \citet{BurknerSurrogate}. As shown in \citet{StuartTeck1}, the normalizing constant implied by 
 \cref{eq:post-approx-EL} is given by
 \begin{equation}
 \int_{\parSpace} \E_{\Ndesign}\left[\postEm[\Ndesign](\Par) \right] d\Par 
 = \E_{\Ndesign} \int_{\parSpace} \postEm[\Ndesign](\Par) d\Par
 = \E_{\Ndesign}\left[\normCstEm \right],
 \end{equation} 
 implying that the expected likelihood approximation is a ratio estimator of the form 
 $\E_{\Ndesign}[\postEm[\Ndesign](\Par)] / \E_{\Ndesign}[\normCstEm]$.
 The following results provide closed-form expressions for $\postApproxMarg[\Ndesign](\Par)$ in 
 the Gaussian settings introduced in \cref{prop:fwd-em-Gaussian} and \cref{prop:llik-em-Gaussian}.
...
\begin{prop} \label{prop:llik-em-Gaussian}
Consider the setting in \cref{prop:llik-em-Gaussian} with a Gaussian log-posterior emulator. The 
expected likelihood approximation is given by 
\begin{align}
\postApproxMarg[\Ndesign](\Par) &= \Exp{\emMean[\Ndesign]{\lpost}(\Par) + \frac{1}{2}\emKer[\Ndesign]{\lpost}(\Par)}.
\end{align}
\end{prop}
...
\begin{align}
&\postApproxMean[\Ndesign](\Par; \fwdEm[\Ndesign]) \Def \postDens(\Par; \emMean[\Ndesign]{\fwd}),
&&\postApproxMean[\Ndesign](\Par; \llikEm[\Ndesign]) \Def \priorDens(\Par) \Exp{\emMean[\Ndesign]{\llik}(\Par)}
\end{align}
\textit{plug-in mean}, or succinctly \textit{mean}, approximations.
...
\subsubsection{Expected Likelihood}
To propagate the surrogate uncertainty, one can consider taking pointwise expectations of 
$\postEm[\Ndesign](\Par)$ with respect to the surrogate predictive distribution. We refer to the 
resulting posteriors
\begin{align}
&\postApproxMarg[\Ndesign](\Par; \fwdEm[\Ndesign]) \Def \E_{\Ndesign}\left[\postDens(\Par; \fwdEm[\Ndesign])\right],
&&\postApproxMarg[\Ndesign](\Par; \lpostEm[\Ndesign]) \Def \priorDens(\Par)\E_{\Ndesign}\left[\Exp{\llikEm[\Ndesign](\Par)}\right],
\end{align}
as \textit{expected likelihood (EL)} approximations, again borrowing terminology from \citet{BurknerSurrogate}.
\footnote{Note that we are technically considering expectations of the unnormalized posterior density approximation 
$\postEm[\Ndesign](\Par)$, but this is equivalent to taking expectations of the likelihood approximation.}

\todo: move EL before plug-in mean. Can then use generic definition $\E_{\Ndesign}\left[\postEm[\Ndesign](\Par) \right]$
without having to define for different emulator cases.



\subsubsection{Other Approximations}
expected log-likelihood, quantile, special case of quantile used in FATES-CES paper
...
\Cref{prop:fwd-em-Gaussian} is stated below. In this case, the distribution of the induced likelihood 
