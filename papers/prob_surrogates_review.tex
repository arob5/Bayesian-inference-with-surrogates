...
In this work, we adopt a generic viewpoint that encompasses both forward model and log-likelihood
emulation. The two approaches each have various strengths
...
The problem of estimating the parameters of differential equation models 
 ...
 stem from the use of dynamical models in Earth system modeling  
...
from the surrogate. Some previous literature assumes this ability, which poses 
no problems for finite-dimensional models (e.g., PCEs and neural networks) but
leads to additional challenges for GPs. For example, the papers 
...
implicitly assume finite-dimensional emulators (see \Cref{sec:post-approx} for details).
We define our use of the terms \textit{finite-dimensional} and 
\textit{infinite-dimensional} below. Let $\funcEm[\Ndesign]$ denote a generic 
...
finite-dimensional parameter $\theta_{\Ndesign}$ exists. In this setting, a generic view of a 
...
meaning the expected likelihood approximation is a ratio estimator of the form 
 ...
 To our knowledge, \citet{BilionisBayesSurrogates} are the first to propose the expected 
 likelihood approximation to propagate surrogate uncertainty. They explore several variants
 of the approximation, which differ based on the treatment of surrogate hyperparameters.
 \citet{SinsbeckNowak,StuartTeck1} follow shortly thereafter, providing a Bayesian decision
 theoretic perspective and lending theoretical support.
