...
\subsection{Acquisition Function Framework}
...
\begin{equation}
\designBatchIn \in \argmin_{\designBatchIn^\prime \in \parSpace^{\Nbatch}} \acq[\Ndesign](\designBatchIn^\prime)
\label{eq:acq-opt}
\end{equation}
with respect to some objective function $\acq[\Ndesign]: \parSpace^{\Nbatch} \to \R$. 
In this context, the objective function is typically called the \textit{acquisition function} or \textit{design criterion}.
...
\begin{algorithm}
	...
	\Comment{Initial Design}	
		...
		\State $\hat{f} \gets \mathrm{update}(\hat{f} \given \designBatchIn, \func(\designBatchIn))$
	\EndFor
	\EndFunction
    \end{algorithmic}
...
updated at each iteration using a new design 
$\{\designBatchIn^{(\designIndex)}, \func(\designBatchIn^{(\designIndex)})\}$
For GPs, these updates
will involve conditioning and potentially also re-fitting of hyperparameters. One could of course
consider varying the batch size $\Nbatch$ across the $\Ndesign$ iterations but to simplify notation
...
be run in parallel. We note that, while the design framework summarized in \Cref{alg:seq_des_loop} 
does allow for consideration of within-batch interactions between design points, 
...
The batch sequential design setting $\Nbatch > 1$ is more
challenging, owing to the fact that $\func$ is only evaluated after an entire batch $\designBatchIn$
has been acquired, meaning that inputs must be selected without observing the responses 
for the other points in the batch. 
