...
that were deemed impractical in \citep{VehtariParallelGP,StuartTeck1}, owing 
to the fact that the latter authors are working with GPs, while the former
...
choices may admit a sufficient statistic which can be emulated independently of the likelihood parameters \Cref{FerEmulation}. 
...
Therefore, in constructing surrogate-induced posterior approximations it is crucial to acknowledge 
the surrogate bias in order to properly calibrate the uncertainty in the posterior 
...
own assumptions and practical considerations. It is important for practitioners to understand these 
differences in order to determine the best approach for a particular application. In this section, we survey 
the various posterior approximations that have been proposed. For practical recommendations in choosing 
a particular method, see \Cref{sec:recs}. 
...
is often not feasible, as it would require impractically large values of $\Ndesign$. This stems from the 
fact that the posterior distribution can be highly concentrated relative to the prior support. Therefore, 
...
learning algorithms target promising regions (according to the current surrogate) but also explore
regions where the surrogate is uncertain.
...
Various papers have claimed that to achieve an accurate posterior approximation, design points
are only required in the region where the posterior is large \citep{AlawiehIterativeGP} [\todo add other citations]. 
This statement should be qualified as follows: achieving an accurate posterior approximation
requires an accurate emulator in the subset of $\parSpace$ where the posterior is large, while also
containing enough global information to know where the posterior is small.
...
a goal-oriented fashion. We focus on active learning strategies
...
Alternative ``maximum uncertainty'' criteria can be defined by varying the measure
of uncertainty (variance, entropy, interquartile range, etc.) and the target quantity
($\postEm[\Ndesign]$, $\funcEm[\Ndesign]$, etc.). For example, targeting the 
...
ECU criteria can be created by changing the measure of uncertainty (e.g., variance, interquartile range, etc.),
the target quantity ($\postEm$, $\funcEm$, etc.), and the weighting measure $\weightDens$.
For example, choosing to assess the variance of $\funcEm$ with uniform weights yields the
...
settings of \Cref{eq:fwd-em-Gaussian,eq:llik-em-Gaussian}.
...
as in \Cref{eq:llik-em-Gaussian}. Then the ECU-variance criterion in \Cref{eq:acq-intvar-saa} reduces to
\begin{equation}
\acq[](\parMat) = \frac{1}{J}
...
$\fwdEm[\Ndesign](\Par) \sim \Gaussian(\emMean[\Ndesign]{\fwd}(\Par), \emKer[\Ndesign]{\fwd}(\Par))$
as in \Cref{eq:fwd-em-Gaussian}, and moreover assume the likelihood is Gaussian.
