...
Motivated by the difficulty of joint optimization, greedy approximations are common in practice.
...
in the pure sequential design setting. The remaining $\Nbatch - 1$ points are selected sequentially via
\begin{align}
&\Par^{(\designIndex, b)} \Def \argmin_{\Par \in \parSpace} \acq[]^{(\designIndex)}(\parMat^{(\designIndex,1:b-1)} \cup \{\Par\}),
&&b = 2, \dots, \Nbatch
\end{align}

An unappealing aspect of the greedy approach is that we are effectively using the batch acquisition function
as if it were a pointwise criterion. 


\subsubsection{Sampling-Based Approaches}


Continuous vs. discrete, greedy opt 
