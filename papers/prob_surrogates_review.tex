...
Emulating the log density as opposed to the density itself is typically preferred as a way to improve numerical stability,
...
result is the analog of \Cref{fwd_em_lik_emulator_moments_Gaussian} in the forward model 
emulation setting.
...
\Var\left[\postEm[\Ndesign](\Par)\right] &= \left[\Exp{\emKer[\Ndesign]{\postDens}(\Par)} - 1\right] \Exp{2\emMean[\Ndesign]{\postDens}(\Par) + \emKer[\Ndesign]{\postDens}(\Par)}
\end{align}
\end{prop}
...
Consider mean approx: \citet{emPostDens,CLMBayesianCalibration}
