...
meaning the expected likelihood approximation is a ratio estimator of the form 
 ...
 decision-theoretic viewpoint as the minimizer of an expected $L^2$ risk. However, this does not 
...
This approach is not directly applicable to nonparametric surrogate models; in the parametric 
setting, it produces samples that are approximately distributed according to 
$\postApproxNormMarg[\Ndesign]$. Though the likelihood approximation is not explicitly defined,
\citet{hydrologicalModel} appear to propose a similar technique.
...
due to their analytic tractability. We summarize these special cases of the expected likelihood below.
...
[\todo: how best to format this? Looks weird.]
\end{prop}

Observe that, conditional on $\Par$, the expected likelihood in this setting is a Gaussian distribution with mean equal 
to the surrogate mean and covariance that is the sum of the observation noise covariance and the GP predictive covariance at $\Par$.
We emphasize that even if the prior $\priorDens$ is Gaussian, the posterior approximation will still be non-Gaussian
due to the nonlinear mean function $\emMean[\Ndesign]{\fwd}(\Par)$, in addition to the fact that the likelihood covariance 
depends on $\Par$ through $\emKer[\Ndesign]{\fwd}(\Par)$. The modified inverse problem viewpoint 
...
may not exist.
The closed-form expression \cref{eq:post-approx-fwd-EL-Gaussian} is also noted in both
...
This result is immediate from \cref{prop:fwd-em-Gaussian}.
...
\paragraph{Gaussian setting: Forward Model Emulation}
...
plug-in mean, expected log-likelihood, quantile, special case of quantile used in FATES-CES paper
