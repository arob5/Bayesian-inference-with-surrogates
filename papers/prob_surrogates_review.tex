...
The assumptions in \cref{llik_em_lik_emulator_moments_Gaussian} are satisfied when either the 
...
&&\lpostEm[0] \sim \GP(\emMean[0]{\postDens}, \emKer[0]{\postDens})
\end{align}
...
or
\begin{equation} 
\lpostEm[\Ndesign] \Def \lpostEm[0]  | [\lpostEm[0](\designIn) = \lpost(\designIn)] \sim \GP(\emMean[\Ndesign]{\postDens}, \emKer[\Ndesign]{\postDens}),
\end{equation}
...
Note that \cref{llik_em_lik_emulator_moments_Gaussian} is agnostic to the specific form of the 
likelihood, unlike \cref{fwd_em_lik_emulator_moments_Gaussian}. However, particular likelihood
...
&= -\frac{\dimObs}{2}\log(2\pi) - \frac{1}{2} \sum_{j=1}^{J} \left[\dimObs_j \log\left(\sigma_j^2\right) +
