...
log posterior density with a statistical surrogate (also called an emulator or meta-model), which induces a computationally cheap approximation of the likelihood.
...
Probabilistic surrogates commonly arise when an emulator is fit using Bayesian methods, though 
alternative approaches such as deep ensembles \citep{deepEnsembles} are increasingly being 
considered. Gaussian processes (GPs; \citet{gpML}) arguably represent the canonical model choice 
in this setting, and have been widely used as emulators in calibrating complex computer codes 
...
are fixed. We discuss inference for such nuisance parameters in \todo.
...
$\state(\Time, \Par) \Def \left[\indexState[1]{\state}(\Time, \Par), \dots, \indexState[\dimState]{\state}(\Time, \Par)\right]^\top \in \mathbb{R}^{\dimState}$,
...
particular application, with common choice sfor the mean including a constant or a linear model of the form 
...
These tradeoffs are related to questions of ensuring well-defined surrogate-induced 
posterior approximations, a topic we discuss in \Cref{sec:existence}.
Practical illustrations are explored in the case study in \Cref{sec:case-study}.
...
\Cref{alg:seq-des-loop} defines a generic sequential design loop for a probabilistic model $\funcPrior$. 
The model is first
...
and those naturally defined in the batch setting. Batch optimization is still possible 
with a single point criterion, which we discuss in \Cref{sec:batch-heuristic}.
...
the posterior approximation. We are not convinced of a single ``correct'' uncertainty 
...
surrogate model itself. Given their generality and ease of implementation, we tend to prefer
the noisy MCMC methods defined in \Cref{sec:MH-approx}; in particular, the expected acceptance
probability and pseudo-marginal methods (recall the latter targets the expected likelihood posterior).
...
results. The expected acceptance probability tends to be more robust with respect to 
heavy-tailed surrogate predictions, but requires stronger requirements to ensure 
existence (\Cref{sec:existence}). We also note that conceptually we find the expected posterior 
approximation to be an attractive choice, but it is often challenging to implement and computationally
expensive. 
...
from the relevant distribution. We similarly compute 
\begin{align}
