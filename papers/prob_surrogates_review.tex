...
(some function of) the expensive computer model is approximated by an emulator, and (ii.) log-density emulation, where
...
approach, as well as typical use cases. The choice between forward model and log-likelihood emulation is also explored
in \citet{StuartTeck1,GP_PDE_priors,random_fwd_models}, and briefly noted in \citet{Surer2023sequential}. We use 
the term \textit{log-density emulation} to encompass both emulation of the log-likelihood and the unnormalized log-posterior density,
as both methods have been used in the literature. 
...
differential equations, and introduce the two regimes of forward model and log-likelihood emulation. The section concludes
with a brief review of Gaussian processes.
...
Consider a \textit{forward model} $\fwd: \R^{\dimPar} \to \R^{\dimObs}$ describing some system of interest, 
parameterized by input parameters $\Par \in \parSpace \subseteq \R^{\dimPar}$. In addition, we suppose that 
...
algorithm. Such algorithms are serial in nature, often requiring $\BigO(10^6)$ iterations, with each 
...
regression models approximating the maps $\Par \mapsto \fwd(\Par)$ or $\Par \mapsto \llik(\Par)$.
...
model the latter as \textit{log-likelihood emulation}. Throughout this review, we will typically view the functions
$\fwd(\Par)$ and $\llik(\Par)$ as computationally expensive black-boxes. We will occasionally refer to 
...
form \ref{ode_inv_prob} motivated by applications to near-term carbon cycle forecasting and 
...
$\gpKerPrior(\designIn)^{-1}$ in \ref{kriging_eqns}. In the present context of deterministic forward models, 
...
This is an example of a stationary, or shift-invariant, kernel since $\gpKerPrior(\Par, \tilde{\Par})$ only depends 
...
predictive distribution \Cref{generic_gp_conditional} at the expense of failing to acknowledge uncertainty
...
\basisWeight_{\idxBasis} &\overset{\textrm{ind}}{\sim} \GP(\emMean[0]{(\idxBasis)}, \emKer[0]{(\idxBasis)}),  \\
...
We begin our review with an overview of commonly-used surrogate models and describe 
the general workflow used to fit these models in order to produce approximations of the expensive simulator.
We emphasize models that provide a probabilistic representation of the emulator uncertainty, in addition
to providing point predictions.
Such considerations have been explored extensively across the much broader landscape of 
surrogate modeling, but we focus the discussion here within the context of surrogate modeling specifically for 
Bayesian inversion. With this in mind, we frame the discussion around
two broad classes of strategies: log-likelihood emulation and forward model emulation.
...
$\{\designIn, \fwd(\designIn)\}$ or $\{\designIn, \llik(\designIn)\}$. In the computer experiments literature, 
...
Ensemble Kalman methods to produce the initial design. We do not discuss these methods further
in this review, but note that further methodological development in combining the strengths of these 
methods may prove a fruitful avenue for future research.
...
Polynomial chaos expansions (PCE) represent a popular alternative to GPs, particularly in
 the engineering community \citep{polyChaosReview,MarzoukPolyChaos}
...
\subsubsection{Relation to computer model calibration literature}
We now begin our discussion of emulation strategies geared towards the goal of Bayesian inversion.
One natural approach is to fit an emulator approximating the forward model $\fwd(\Par)$. 
The challenge of emulating a black-box computer model has received widespread attention 
well beyond the scope of Bayesian inference, and thus the model building process for constructing
such emulators may be informed by the vast literature from the computer experiments, engineering,
and statistics communities. As a starting point, we refer readers to 
\citet{gramacy2020surrogates,design_analysis_computer_experiments,SanterCompExp,UQpredCompSci} 
and the references therein. We take a moment to clarify the scope of our review in relation 
to related work in the computer experiments literature.  
Indeed, early work in this literature addressed the challenge of learning 
...
true underlying system. For example, the pioneering work \citet{KOH} considers jointly learning 
...
focus on the alternative modular \citep{modularization} workflow, where the surrogate is fit offline 
without seeing the calibration data $\obs$. This modular two-stage approach naturally leads to 
...
\subsubsection{Induced Likelihood Approximation}
We let $\fwdEm[\Ndesign]$ denote a probabilistic surrogate model for $\fwd$, which has been
fit using training data $\{\designIn, \fwd(\designIn)\}$ from $\Ndesign$ design points.
Plugging the random function $\fwdEm[\Ndesign]$ in place of $\fwd$ in the log-likelihood induces 
a random approximation of the log-likelihood, which we denote by 
$\llikEmFwd(\cdot) \Def \llik(\cdot; \fwdEm[\Ndesign])$. The induced distribution for 
$\llikEmFwd$ will depend on the predictive distribution of the forward model emulator, as well as 
the structure of the likelihood function. The following section gives details for a special case where
the first two moments of $\llikEmFwd$ are known in closed-form.
...
been considered frequently in the literature (\todo: cite).
...
for all $\Par \in \parSpace$. Then, in the additive Gaussian likelihood setting, the induced likelihood 
surrogate $\Exp{\llikEmFwd(\Par)} = \Gaussian(\obs| \fwdEm[\Ndesign](\Par), \likPar)$ has mean and 
variance given by
\begin{align}
...
and facilitates the presentation of analytical expressions (such as \Cref{fwd_em_lik_emulator_moments_Gaussian})
...
then the independent GP approach may be infeasible or ill-advised. Such complications commonly 
arise in settings with spatial or temporal structure, such as epidemic modeling \citep{FadikarAgentBased},
engineering design \citep{PODemulation}, and ecological forecasting (\todo: cite).  
In such contexts, a popular 
alternative is to leverage the basis reduction strategy \ref{basis_func_model}. The surrogate predictive 
distribution remains Gaussian under this model, and thus \Cref{fwd_em_lik_emulator_moments_Gaussian}
still applies in this setting. The particular setup of a Gaussian likelihood with a GP forward model emulator 
using the basis vector approach is explored in \citet{Surer2023sequential}. We also briefly note that 
a great deal of literature has addressed the challenge of emulating forward models with dynamical structure using 
methods other than the basis vector approach; e.g., 
\citet{GP_dynamic_emulation, Bayesian_emulation_dynamic, Liu_West_dynamic_emulation, dynamic_nonlinear_simulators_GP}.
...
This implies fitting a surrogate model to the design $\{\designIn, \llik(\designIn)\}$ or $\{\designIn, \lpost(\designIn)\}$, respectively.
We refer to these approaches collectively as \textit{log-density} emulation.
The log-likelihood emulation approach is considered in 
\citet{VehtariParallelGP,FATES_CES,trainDynamics,quantileApprox,ActiveLearningMCMC,FerEmulation,StuartTeck1,random_fwd_models,GP_PDE_priors}.
A related approach is explored in \citep{wang2018adaptive}. 
...
The closely-related unnormalized log-posterior density (i.e., the joint parameter-data density) emulation
strategy is used in \citet{Kandasamy_2017,llikRBF,gp_surrogates_random_exploration,landslideCalibration}.
We colloquially refer to this latter approach as log-posterior emulation for succinctness.
Emulating the log density over the density itself is typically preferred as a way to improve numerical stability,
...
but the resulting response surface may prove more challenging to emulate. This approach also increases both the 
...
The following sub-section provides such an example for multiplicative Gaussian likelihoods, following the 
approach of \citet{FerEmulation}.

\subsubsection{Log-Likelihood vs. Log-Posterior Emulation}
...
\subsubsection{Gaussian Setting}
\Cref{fwd_em_lik_emulator_moments_Gaussian} demonstrates that the predictive moments of the induced likelihood 
surrogate are available in closed-form when both the likelihood and the surrogate predictive distribution are 
Gaussian. In the setting of log-density emulation, the distribution of the induced likelihood surrogate can be
fully characterized for Gaussian surrogates.

\begin{prop} \label{llik_em_lik_emulator_moments_Gaussian}
Assume that $\lpostEm[\Ndesign]$ is a log-posterior emulator with Gaussian predictive distribution; 
that is, $\lpostEm[\Ndesign](\Par) \sim \Gaussian(\emMean[\Ndesign]{\postDens}(\Par), \emKer[\Ndesign]{\postDens}(\Par))$
...
\begin{align}
\postEm[\Ndesign](\Par) &\sim \text{LN}\left(\emMean[\Ndesign]{\postDens}(\Par), \emKer[\Ndesign]{\postDens}(\Par)\right),
\end{align}
which implies 
\begin{align}
\E\left[\postEm[\Ndesign](\Par)\right] &= \Exp{\emMean[\Ndesign]{\postDens}(\Par) + \frac{1}{2}\emKer[\Ndesign]{\postDens}(\Par)},
\end{align}
\end{prop}

 It is typically preferable to focus on approximating the logarithm 
...
in , which focuses on the Gaussian likelihood \ref{llik_Gaussian} in the specific case $\likPar = \sigma^2 I$.
In this 
...
Include likelihood surrogate distribution under GP.
Difficulties: non-stationarity (cite Booth/Gramacy nonstationary surrogate review), bound constraints (cite quantile/bound constraint paper)
Note that the unnormalized log posterior density could also be modeled (cite Sanz-Alonso paper)
Issue with likelihood parameters (cite Wild paper)
Include 1d example.
...
Present the general marginal posterior. Cite Burkner, and discuss how computation takes the form of
an ensemble of MCMC runs. Then discuss how this runs into issues with nonparametric emulators.
...
% An Overview of Surrogate modeling in Bayesian inference 
...
\Cref{alg:sample-approx} [\todo: subscript notation indexing samples in algorithm might 
...
\subsubsection{Integrated Conditional Uncertainty}
We now consider specific instantiations of the integrated conditional uncertainty criterion \ref{acq_int_cond_unc}
that are adapted to the goal of posterior approximation. Under \Cref{acq_int_cond_unc}, the selection of 
$\qoi$ and $\weightDens$ are the two design decisions at our disposal to construct such criteria. In other 
words, we consider specific choices of either the integrand or the measure defining the integral.
...
This derivation leverages \Cref{lemma:pred-mean-dist}, which characterizes the distribution of 
...
\textit{variance inflation factor}, accounts for the uncertainty in the unobserved response
 $\designBatchLlik$. As in \Cref{lemma:evar}, the variance inflation is larger when the 
...
in \Cref{sec:post_approx}, which implies weighting the integral over $\parSpace$ by 
...
\Cref{sec:post_approx}. [\todo: need name and/or notation for acquisitions of this form]
