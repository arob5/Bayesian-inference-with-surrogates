...
\begin{algorithm}
    ...
    \Function{MH}{$\indexMCMC[0]{\Par}, \NMCMC$}     
        \For{$\mcmcIndex \gets 1, \dots, \NMCMC$} 
        		\State $\propPar \sim \propDens(\indexMCMC{\Par}, \cdot)$
		\State $\accProbMH(\indexMCMC{\Par}, \propPar) \gets \min\left\{1, \frac{\postDens(\propPar)\propDens(\propPar, \indexMCMC{\Par})}{\postDens(\indexMCMC{\Par}) \propDens(\indexMCMC{\Par}, \propPar)} \right\}\right\}$
...
We consider a family of algorithms that approximate $\accProbMH(\Par, \propPar)$ by
replacing $\postDens(\Par)$  and $\postDens(\propPar)$ in \Cref{MH_acc_prob_exact}
with approximations that are sampled from a specified distribution. The particular choice of 
...
to target $\R^2_{+}$. The generic noisy MH algorithm is stated in \Cref{alg:MH-noisy}. Consider 
the following three special cases.
 ...
 \begin{algorithm}
    ...
    \label{alg:MH-noisy}
    \begin{algorithmic}[1]
    \Function{MH-Noisy}{$\indexMCMC[0]{\Par}, \NMCMC, \postSampleKernel$}
    	\State $\hat{\postDens}_{\text{curr}} \sim \mathrm{law}(\postEm[\Ndesign](\indexMCMC[0]{\Par}))$
        \For{$\mcmcIndex \gets 1, \dots, \NMCMC$} 
        		\State $\propPar \sim \propDens(\indexMCMC{\Par}, \cdot)$
		\State $[\hat{\postDens}_{\indexMCMC{\Par}}, \hat{\postDens}_{\tilde{\Par}}] \sim \postSampleKernel([\indexMCMC{\Par}, \propPar, \hat{\postDens}_{\text{curr}}],\cdot)$
		\State $\hat{\accProbMH} \gets \min\left\{1, \frac{\hat{\postDens}_{\tilde{\Par}} \cdot \propDens(\propPar, \indexMCMC{\Par})}{\hat{\postDens}_{\indexMCMC{\Par}} \cdot \propDens(\indexMCMC{\Par}, \propPar)} \right\}\right\}$
		\State $b \sim \text{Bernoulli}(\hat{\accProbMH})$
		\If{$b = 1$}
			\State $\indexMCMC[\mcmcIndex+1]{\Par} \gets \propPar$
			\State $\hat{\postDens}_{\text{curr}} \gets \hat{\postDens}_{\tilde{\Par}}$
		\Else
			\State $\indexMCMC[\mcmcIndex+1]{\Par} \gets \indexMCMC{\Par}$
		\EndIf
	\EndFor
...
\begin{minipage}[t]{0.495\textwidth}
    ...
    \end{algorithmic}
\end{minipage}
\caption{Side-by-side comparison of Metropolis-Hastings and Noisy Metropolis-Hastings algorithms.}
\end{figure}
...
In principle, it thus provides an exact MCMC scheme to sample from the expected likelihood 
...
in the estimate of from $\E_{\Ndesign}[\postEm[\Ndesign](\Par)]$; e.g., by increasing $J$ in
...
though the efficiency of MCwMH is typically less sensitive to the variance in the estimate
...
as an approximation to $\postApproxNormMarg[\Ndesign]$. A variant of this algorithm 
