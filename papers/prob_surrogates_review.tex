...
methods (e.g., MCMC), thus representing a Metropolis within Monte Carlo 
approach \citep{garegnani2021NoisyMCMC}.
...
of $\postNormEm[\Ndesign]$, which will generally be possible for finite-dimensional 
...
As with \citet{trainDynamics}, this approach only approximates \cref{alg:ep}.

[\todo: check \citet{trainDynamics} again; they might not be trying to implement the direct sampling 
approach, but rather sampling trajectories within a single MCMC scheme.]
...
In this section we demonstrate that the surrogate-induced posteriors introduced above can be viewed as
invoking particular approximations of the acceptance probability in a Metropolis-Hastings (MH) MCMC 
algorithm. This perspective yields a family of posterior approximation algorithms that take the MH acceptance
probability, as opposed to the unnormalized posterior density, as the target quantity for approximation.
This approach is proposed in \citet{surrogateNoisyMCMC}, which we refer to for additional details.
...
\begin{algorithm}
    ...
    \Function{MH}{$\indexMCMC[0]{\Par}, \NMCMC$}     
		...
		\State $\accProbMH(\indexMCMC{\Par}, \propPar) \gets \min\left\{1, \frac{\postDens(\propPar)\propDens(\propPar, \Par)}{\postDens(\Par) \propDens(\Par, \propPar)} \right\}\right\}$
...
We will now replace $\accProbMH(\Par, \propPar)$ with a surrogate-induced approximation.
As previously, we consider a generic random surrogate $\postEm[\Ndesign]$ for the 
unnormalized posterior density, induced by an underlying emulator. By inserting $\postEm[\Ndesign]$
in place of $\postDens$ in $\accProbMH(\Par, \propPar)$, the surrogate induces a random approximation
\begin{equation}
...
\end{equation}
of the MH acceptance probability. Notice that $\accProbMHEm[\Ndesign](\Par, \propPar)$ depends on the 
bivariate surrogate predictive distribution $[\postEm[\Ndesign](\Par), \postEm[\Ndesign](\propPar)]$. Therefore, 
choosing the acceptance probability as the approximation target offers the opportunity for incorporating 
joint structure from the surrogate predictive distribution, as opposed to approximations derived via 
pointwise approximations of the unnormalized posterior density. \citet{surrogateNoisyMCMC} propose 
performing approximate inference by running the MH algorithm using the expected acceptance probability
$\E_{\Ndesign}\left[\accProbMHEm[\Ndesign](\Par, \propPar)\right]$ in place of $\accProbMH(\Par, \propPar)$. 
A practical algorithm is given in \cref{alg:MH-noisy}, which proceeds by drawing a sample of 
$\accProbMHEm[\Ndesign](\Par, \propPar)$ at
 each iteration. In addition to incorporating the joint structure of the surrogate, this algorithm has the 
 benefit of only requiring the ability to draw samples from the joint distribution of
 $[\postEm[\Ndesign](\Par), \postEm[\Ndesign](\propPar)]$. It thus does not rely on Gaussian assumptions, 
 nor the requirement to approximate infinite-dimensional sample trajectories. 
 
 \begin{algorithm}
    \caption{Metropolis-Hastings with Expected Acceptance Probability}
    \label{alg:MH-noisy}
    \begin{algorithmic}[1]
    \Function{MHApprox}{$\indexMCMC[0]{\Par}, \NMCMC$}     
        \For{$\mcmcIndex \gets1,\NMCMC$} 
        		\State $\propPar \sim \propDens(\indexMCMC{\Par}, \cdot)$
		\State $\accProbMH^\prime \sim \mathrm{law}\left(\accProbMHEm[\Ndesign](\Par, \propPar)\right)$
		\State $b \sim \text{Bernoulli}(\accProbMH^\prime)$
		\If{$b = 1$}
			\State $\indexMCMC[\mcmcIndex+1]{\Par} \gets \propPar$ 
		\Else
			\State $\indexMCMC[\mcmcIndex+1]{\Par} \gets \indexMCMC{\Par}$
...
\Cref{alg:MH-noisy} can alternatively be viewed as sampling a trajectory from $\postEm[\Ndesign]$
at each iteration, then projecting onto the finite-dimensional distribution at the inputs $(\Par, \propPar)$.
Given this perspective, \citet{surrogateNoisyMCMC} discuss how \cref{alg:MH-noisy} can be 
viewed as an approximation to the expected posterior sampling scheme in \cref{alg:ep}.
The algorithm also bears a close resemblance to pseudo-marginal MCMC
\citep{pseudoMarginalMCMC,noisyMCMC,noisyMCSurvey}. Indeed, if \cref{alg:MH-noisy}
is modified so that only $\postEm[\Ndesign](\propPar)$ is sampled at each iteration, with 
the value for $\postEm[\Ndesign](\Par)$ recycled from the previous iteration, then 
the result is a pseudo-marginal MCMC algorithm with target distribution $\postApproxNormMarg[\Ndesign]$
\citet{surrogateNoisyMCMC} .
...
introduced above are well-defined. This theoretical technicality points to important practical 
issues in constructing emulators for use in posterior approximation. The primary concern
is that the tail behavior of the surrogate may lead to approximations of $\postDens(\Par)$ that 
...
increases. Table \todo illustrates simple examples of such cases. In order to avoid issues related to
...
In \citet{random_fwd_models}, this assumption is relaxed and various existence conditions are 
established for generic random surrogates and general $\parSpace$.
