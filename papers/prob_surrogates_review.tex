...
$\Par \Def \{\KEXT, \fracRespiration, \tauV, \dstateV(0)\}$ with the remaining parameters held fixed. Figure \todo summarizes
variation in the three state trajectories as $\Par$ varies.
...
\end{equation}
and the remaining entries simply change the indices of the summation. For simplicity, we fix $\likPar$
and assume a priori independence over the entries of $\Par$. Synthetic data $\obs$ is simulated
using \Cref{eq:vsem-inv-prob} with fixed ground truth values for $\Par$ and $\likPar$. The same 
model is used when solving the inverse problem, but the values of the fixed parameters (those not
being estimated) and the noise covariance are changed, implying the presence of parametric misspecification.
Table \todo compares these misspecified values relative to the ground truth, while
table \todo summarizes the prior distribution $\Par \sim \priorDens$. 
