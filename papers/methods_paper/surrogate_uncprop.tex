...
quality of the random kernel pCN (rk-pcn) approximation to the EP. For the latter, we evaluate the
rk-pcn schemes in \Cref{alg:rk-pcn-finite,alg:rk-pcn-infinite} at different values of the 
...
closed-form EP and EUP distributions. Each of the rk-pcn algorithms accurately represent the EP, with the $\rho = 0.99$
...
parameter space in this example. The rk-pcn algorithm is well-equipped for this situation, as the pCN proposal is 
...
\label{eq:pde-inv-prop}
\end{equation}
While the effects of the spatial discretization and prior approximation are of interest, we neglect such questions 
and treat \Cref{eq:pde-inv-prop} as the baseline ``exact'' model in order to focus on questions of surrogate uncertainty propagation.
