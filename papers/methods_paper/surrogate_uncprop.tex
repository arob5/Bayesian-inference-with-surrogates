...
\end{equation}
Henceforth, we consider the particular case where $\emDist$ is a Gaussian measure. 
\footnote{More generally, the emulator predictive distribution itself need not be Gaussian. By simply re-defining
$\target$, $\emDist$ can be the pushforward of an underlying Gaussian measure.}
This is typical in the infinite-dimensional setting we seek to address. Generalization to the non-Gaussian setting is 
straightforward when $\emDist$ is finite-dimensional.
