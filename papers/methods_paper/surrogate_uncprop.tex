...
\Cref{sec:conclusion} concludes. Proofs, derivations, and additional technical details
...
Gaussian processes (GPs), Bayesian neural networks, deep ensembles, and Bayesian additive regression trees.
\todo{add citations}
...
The surrogate-approximated analogs $\postDens(\cdot; \targetEm)$, $\normCst(\targetEm)$, and 
...
theoretic viewpoint and select the optimal distribution from a set of candidates $\qSpace$ that 
...
well as the naive cut algorithm described in \citet{PlummerCut}.
