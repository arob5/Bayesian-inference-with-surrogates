...
have been suggested, with little guidance on how their performance can vary.
To fill this gap, we start by proposing a particular baseline uncertainty-aware posterior
approximation, justified by decision theoretic and modular Bayesian inference arguments.
We then investigate a popular computationally convenient alternative, highlighting when 
it may differ from the baseline. Finally, we develop a Markov chain Monte Carlo
algorithm that is shown to produce a superior approximation to the baseline across a variety
of experiments. Our numerical experiments are motivated by problems of parameter 
estimation for dynamical models, with particular emphasis on Gaussian process surrogates
trained to emulate either the underlying forward model or the log likelihood of the Bayesian model.
\end{abstract}
...
implementations of the former approach, and provide a characterization of the approximation
error for a commonly-used heuristic. We then adopt the latter perspective,
introducing a new approximate MCMC algorithm that is shown to outperform 
competing approaches.
 
\paragraph{Contributions.}
\begin{enumerate}
\item We formalize the problem of surrogate uncertainty propagation in Bayesian inverse problems.
Adopting a Bayesian decision theoretic framework, we identify a particular mixture distribution, 
termed the \textit{expected posterior (EP)}, as the optimal target for approximate inference.
\item We analyze a popular heuristic, the \textit{expected unnormalized posterior (EUP)},
providing results that elucidate when the EUP and EP may diverge.
\item We introduce an approximate preconditioned Crank-Nicolson MCMC algorithm that 
directly targets the EP, which is shown to perform favorably against alternatives.
\end{enumerate}
...
prediction of $\target(\Par)$. \Cref{ex:fwd-em} and \Cref{ex:ldens-em} describe the common strategies 
...
i.e., $\targetEm(\Par)$ is a random vector, implying $\targetEm$ is a random function.
...
due to the limited computational budget. Common models that provide predictive distributions include
...
has been shown to outperform the joint Bayesian approach in the presence of model misspecification
...
Note that the plug-in mean, EP, and EUP can all be written in this mixture form with the respective
weights $\delta_{\emE[\targetEm]}(\d\targetTraj)$, $\emDist(\d\targetTraj)$, and $\emDist(\d\targetTraj \given \obs)$.
...
Under these assumptions, the pushforward predictive distributions for $\postEm(\Par)$ are log-normal
...
MCMC algorithm that produces approximate EP samples. Our method avoids the challenges 
associated with discretization and repeated just-in-time sampling.
The sampler targets the joint distribution 
...
\begin{align}
\nextstate{\targetTraj} &\sim \MarkovKerTarget(\targetTraj, \cdot) \label{eq:f-update} \\
\nextstate{\Par} &\sim \MarkovKerPar(\Par, \cdot \given \nextstate{\targetTraj}), \label{eq:u-update}
\end{align}
...
to the exact kernel $\MarkovKerPar(\Par, \cdot \given \target)$.
...
$\propPar \sim \propDistPar(\Par, \cdot)$ and setting the next state $\nextstate{\Par}$ to $\propPar$
with probability 
\begin{equation}
\accProbMH_{\Par}(\Par, \propPar)
= \min\left(1, \frac{\postDens(\propPar; \nextstate{\targetTraj})}{\postDens(\Par; \nextstate{\targetTraj})} \cdot 
\frac{\propDensPar(\propPar, \Par)}{\propDensPar(\Par, \propPar)} \right);
\label{eq:MH-ratio-u}
\end{equation}
otherwise $\nextstate{\Par}$ is assigned the current state $\Par$. Here $\propDensPar(\Par, \cdot)$ denotes the density of 
$\propDistPar(\Par, \cdot)$. Provided the unnormalized density $\postDens(\Par; \nextstate{\targetTraj})$
depends on $\nextstate{\targetTraj}$ only through $\nextstate{\targetTraj}(\Par)$, then the MH update for $\Par$ only requires 
realizing the finite-dimensional projection $[\nextstate{\targetTraj}(\Par), \nextstate{\targetTraj}(\propPar)]$ of the 
function $\nextstate{\targetTraj}$.
...
approximation through experiments; a complete theoretical analysis is beyond the scope of the present paper.
...
\begin{algorithm}[H]
    ...
    \State $\nextstate{\Par} \gets \propPar \indicator{v \leq \alpha_{\Par}} + \Par \indicator{v > \alpha_{\Par}}$ \Comment{$\Par$ update}
    \State $\nextstate{\targetTraj}_{\nextstate{\Par}} \gets \nextstate{\targetTraj}_{\propPar} \indicator{v \leq \alpha_{\Par}} + \nextstate{\targetTraj}_{\Par} \indicator{v > \alpha_{\Par}}$
    \end{algorithmic}
\end{algorithm}

\begin{rmk}
The approximate Metropolis-within-Gibbs scheme above is derived by invoking the approximation
$\postDensNorm(\Par; \tilde{\targetTraj}) / \postDensNorm(\Par; \targetTraj) \approx 1$.
...
Addressing pathologies in this approach were a primary motivation for our proposed method. 
...
quality of the random kernel pCN (RKpCN) approximation to the EP. For the latter, we evaluate the
RKpCN schemes in \Cref{alg:rk-pcn-finite,alg:rk-pcn-infinite} at different values of the 
correlation parameter $\pcnCor$. We compare these MCMC approximations against the EUP as 
well as a version of the naive cut algorithm described in \citet{PlummerCut}.
\todo[inline]{need to more precisely describe this comparison baseline; here or in appendix?}
...
\end{align*}
\end{prop}
\todo[inline]{how best to format this? looks awkard}
...
\begin{equation}
\source(\state) = \sum_{i=1}^{4} \frac{0.8}{\delta\sqrt{2\pi}} \exp\left\{-\frac{1}{2\delta^2}(\state - c_r)^2 \right\}
\end{equation}
...
$\log \perm(X) \sim \Gaussian(1, k_\kappa(X, X))$ for the discretized model. 
...
spatial inverse problems \citep{uribeKL,dimRedPolyChaos}. Letting $\{(\lambda_r, \varphi_r)\}_{r=1}^{6}$ denote 
the dominant eigenpairs, we thus adopt the rank-reduced prior model 
\begin{equation}
\log \perm(X) \Def 1 + \sum_{r=1}^{6} \sqrt{\lambda_r} \Par_j \psi_j, \qquad \Par_j \overset{\mathrm{iid}}{\sim} \Gaussian(0, 1),
\end{equation}
noting that the six parameters $\Par \Def (\Par_1, \dots, \Par_6)^\top$ now control the permeability field.
...
\end{equation}
While the effects of the spatial discretization and prior approximation are of interest, we neglect such questions 
...
\paragraph{Surrogate Model.}
We fit a GP to target the forward model $\target \Def \fwd$, consisting of independent single-output GPs fit to predict each of
the four outputs. The training data $\{(\Par_n, \target(\Par_n))\}_{n=1}^{\Ndesign}$ is constructed by sampling 
a latin hypercube design from the unit hypercube, mapping the points through the standard Gaussian 
...
behavior stemming from the GP surrogate's reversion to the prior in the tails. Similar practical tactics are
...
is repeated for different numbers of design points $\Ndesign \in \{10, 30, 50\}$. Inference for the exact, 
plug-in mean, and EUP distributions is conducted via Metropolis-Hastings, retaining 5000 samples 
each after dropping burn-in and thinning. Approximate EP samples are obtained using the RKpCN algorithm
in \Cref{alg:rk-pcn-infinite}. It is difficult to obtain an exact EP baseline in this higher dimensional example
...
\citet{pathwiseConditioning,samplingGPPosts}, with the GP prior term approximated using 1000 random 
