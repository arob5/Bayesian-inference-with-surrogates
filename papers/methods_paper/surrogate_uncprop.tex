...
approximations to the MCwM EP baseline. We use entropic-regularized 2-Wasserstein distance,
...
the MCwM so that the distances are computed in the baseline MCwM geometry. The Sinkhorn regularization parameter is 
selected using the \citep{ottjax} JAX applied to the MCwM points, and the same value is used for all Wasserstein computations
for a fixed design size. The results show that both RKpCN with $\rho = 0.99$ and the EUP provide the best EP approximations, 
with the median EUP replicate performing slightly better. 
...
can significantly deviate when the surrogate-based posterior density approximation approximation has highly non-uniform
