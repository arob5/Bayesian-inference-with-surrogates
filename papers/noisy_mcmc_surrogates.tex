...
\end{equation}
but this ignores the emulator uncertainty, leading to overconfident 
...
generative model in \Cref{eq:ep-prob-model}. The following result shows that 
...
expectation in \Cref{eq:ep-approx} is well-defined in the case that $\targetEm$ is an 
...
sampling procedure in \Cref{eq:ep-prob-model}.
...
both the EP and EUP in their examples. Based on our results in \Cref{sec:EP}, 
...
Notice from \Cref{eq:post-approx-EL} that the EUP is a ratio estimator, and differs from 
...
= \emE[\postDens(\Par; \targetEm)] \jgap + \Cov[\postDens(\Par; \targetEm), \normCst(\targetEm)],
\label{eq:ep-eup-pw-err}
\end{equation}
where $\jgap \Def \emE[\normCst^{-1}(\targetEm)] - \emE[\normCst(\targetEm)]^{-1}$
...
$\emE[\postDens(\Par; \targetEm)]$. Since both distributions integrate to one, any positive 
...
the EUP to be more peaked, while the EP is more smooth. 

Decreasing the variance of $\normCst(\targetEm)$ will shrink both terms in 
\Cref{eq:ep-eup-pw-err} for all $\Par$ values. This may occur when there is little 
...
in $\postDens(\cdot; \targetTraj)$. In the first case, note that if $\funcEm$ is heavily 
