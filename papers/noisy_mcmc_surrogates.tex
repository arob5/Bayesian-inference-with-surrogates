...
Assuming a prior density $\priorDens(\Par)$ and likelihood $\lik(\Par; \obs)$, 
...
Let $\postDens(\Par) \Def \priorDens(\Par) \lik(\Par; \obs)$ denote the unnormalized posterior 
...
In large-scale applications, computationally constraints impose a budget of 
...
from an ensemble of posterior approximations, each induced by a single trajectory of $\targetEm$.
...
Despite these similarities, the surrogate setting features a few key differentiating characteristics.
...
$\E_{\Ndesign}\left[\postEm(\Par) \right]$. This includes the use of GP forward
...
\end{equation}
which still considers this dependence.
