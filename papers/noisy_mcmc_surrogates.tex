...
\Cref{eq:ep-eup-pw-err} to blow up. This can result in $\postApproxNormMarg$ concentrating 
...
``uncertainty inflation factor'' is given by
 ...
 \end{equation}
in this setting. We conclude from \Cref{eq:unc-infl-factor}
...
then \Cref{eq:unc-infl-factor} reduces to $\Exp{\frac{3}{2}\gpKer(\Par)}$. If $\gpKer(\Par) = 4$, this means
...
$\emDist(d\targetTraj) \postDensNorm(\Par; \targetTraj) d\Par$, which admits
...
update must leave the distribution $\emDist(d\targetTraj)\postDensNorm(\Par; \targetTraj)$
...
$\postDensNorm(\Par; \targetTraj)$. The former can be addressed by so-called 
...
$\propDist(\targetTraj, \cdot)$ then the Metropolis acceptance probability reduces to
\begin{equation}
