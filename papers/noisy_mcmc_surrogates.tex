...
We now consider the particular example where $\postEm(\Par) = \Exp{\funcEm(\Par)}$
and $\funcEm \sim \GP(\gpMean, \gpKer)$ is a GP emulator; that is, 
$\E[\funcEm(\Par)] = \gpMean(\Par)$ and 
...
For example, when the standard deviation of $\funcEm(\Par^\prime)$ is twice that of $\funcEm(\Par)$,
...
$\funcEm \sim \GP(\gpMean, \gpKer)$. This setup has been considered frequently in various 
