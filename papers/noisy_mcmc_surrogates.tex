...
within the physical, biological, and engineering sciences. These models often require specifying 
values for uncertain input parameters in order to tune the model for the application of interest. 
A common approach is to calibrate model parameters to align simulation outputs with observational data.
This is naturally cast as a statistical parameter estimation problem, and a Bayesian framework is commonly
employed to quantify parameter uncertainty. However, standard Bayesian inference algorithms such 
as Markov chain Monte Carlo (MCMC) are often hindered by the computational cost of the simulation model.
A popular approach to deal with this issue is to train a surrogate model (i.e., emulator) as a computationally
thrifty approximation of the expensive computer code. A canonical surrogate choice is a Gaussian 
process (GP), which provides a probabilistic prediction of the simulation output at any input parameter 
value. There has been increasing interest in methods for leveraging the [\todo: finish intro]
...
observations $\obs \in \obsSpace \subseteq \R^{\dimObs}$. Adopting a Bayesian viewpoint,
we assume prior knowledge is encoded by a density $\priorDens(\Par)$. The goal is then to 
...
We let $\postDens(\Par) \Def \priorDens(\Par) p(\obs \given \Par)$ denote the unnormalized posterior 
density, which plays a central role in surrogate-based Bayesian inference. 
...
Our focus in this work is on methods that overcome this computational bottleneck by replacing 
the expensive simulator with a computationally thrifty approximation, which we refer to synonymously 
as a \textit{surrogate} or \textit{emulator} model. Our interest is in \textit{probabilistic} surrogate models, 
which output predictions in the form of a probability distribution, as opposed to just point estimates.
In the following, we motivate this setup by introducing common surrogate modeling strategies used 
in solving Bayesian inverse problems, and describe how each of these strategies induces a random 
approximation of the density $\postDens(\Par)$.
...
the likelihood often takes the form $\obs = \fwd(\Par) + \noise$ for some \textit{parameter-to-observable map}
...
After 
normalizing $\postEm[\Ndesign]$, the random density  
\begin{align}
&\llikEmRdm[\Ndesign]{\postDensNorm}(\Par) 
\Def \frac{1}{\llikEmRdm[\Ndesign]{\normCst}} \llikEmRdmDens[\Ndesign](\Par),
&&\llikEmRdm[\Ndesign]{\normCst} \Def \int_{\parSpace} \llikEmRdmDens[\Ndesign](\Par)d\Par.
...
The work \citet{BurknerSurrogate} studies \cref{alg:ep} in the context of finite-dimensional
...
\begin{align}
&\llikEmMeanDens[\Ndesign](\Par; \fwdEm[\Ndesign]) \Def \postDens(\Par; \E[\fwdEm[\Ndesign]]),
...
\end{align}
as \textit{expected likelihood} approximations, again borrowing terminology from \citet{BurknerSurrogate}.
...
\midrule
Plug-In Mean & 
\(\displaystyle \min\left(1, \Exp{\E[\lpostEm[\Ndesign](\propPar)] - \E[\lpostEm[\Ndesign](\Par)]} \right)\) \\ 
Expected Likelihood & 
\(\displaystyle \min\left(1, \E \left[\Exp{\lpostEm[\Ndesign](\propPar)}\right] / \E\left[\Exp{\lpostEm[\Ndesign](\Par)}\right] \right)\) \\
Expected Likelihood Ratio & 
\(\displaystyle \min\left(1, \E\left[\Exp{\lpostEm[\Ndesign](\propPar) - \lpostEm[\Ndesign](\Par)} \right] \right)\) \\
Expected Acc. Prob. & 
\(\displaystyle \E\left[\min\left(1, \Exp{\lpostEm[\Ndesign](\propPar) - \lpostEm[\Ndesign](\Par)}\right)\right]\) \\
\bottomrule
\end{tabular}
\caption{The approximations to the Metropolis-Hastings (MH) acceptance probability implied by different posterior approximations. All
expectations are with respect to the underlying surrogate predictive distribution. For brevity, the acceptance probabilities 
...
% Emphasize asymptotics: gp_surrogates_random_exploration, StuartTeck1, StuartTeck2
% Stepwise uncertainty reduction: weightedIVAR
% Use GP only for proposal: MCMC_GP_proposal
% Design occurs within MCMC: Li_2014, ConradLocalExactMCMC, ActiveLearningMCMC
% Review this one: random_fwd_models

% Maybe cite (after reading more closely): VillaniAdaptiveGP, Semler_2023, Semler2024GradientEnhanced, villani2024posteriorsamplingadaptivegaussian

% On the Sampling Problem for Kernel Quadrature: talk about oversampling the tails (does this align with my recent design ideas?)
