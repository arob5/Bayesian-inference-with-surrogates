...
\begin{algorithm}
    ...
    \Function{sampleEP}{$\llikEmRdm[\Ndesign]{\postDensNorm}, \NSample, M$}     
        \For{$\sampleIndex \gets 1, \dots, \NSample$} 
        		\State $\func^{(\sampleIndex)} \sim \mathrm{law}(\funcEm)$ \Comment{Sample emulator trajectory}
		\State $\Par^{(\sampleIndex, 1)}, \dots, \Par^{(\sampleIndex, M)} \overset{iid}{\sim} \postDensNorm(\cdot; \func^{(\sampleIndex)})$ \Comment{Sample posterior given trajectory}
	\EndFor
...
We analyze these special cases in \Cref{sec:fwd-Gaussian,sec:ldens-Gaussian}.
...
$\funcEm \sim \GP(\gpMean, \gpKer)$, such that $\E[\funcEm(\Par)] = \gpMean(\Par)$ and 
$\Cov[\funcEm(\Par),\funcEm(\Par^\prime)] = \gpKer(\Par, \Par^\prime)$ for all 
$\Par, \Par^\prime \in \parSpace$. This setup has been considered frequently in various 
...
$\postEm(\Par) = \priorDens(\Par) \Gaussian(\obs \mid \funcEm(\Par), \likPar)$, implying
\begin{equation}
\postApproxNormMarg(\Par) \propto \Gaussian(\obs \mid \gpMean(\Par), \likPar + \gpKer(\Par)),
\label{eq:EL-Gaussian-fwd}
\end{equation}
following from the formula for the convolution of two Gaussians [\todo: add to appendix].
We have used the shorthand $\gpKer(\Par) \Def \gpKer(\Par, \Par)$. A key observation 
in this setting is that $\postEm(\Par) \in [0, B_{\likPar}]$, where 
...
In general, it is tempting to view the EL as inflating the posterior approximation in regions
where surrogate uncertainty is larger. However, this is not strictly the case in this context.
Indeed, consider the plug-in mean approximation 
$\postApproxNormMean(\Par) \propto \priorDens(\Par) \Gaussian(\obs \mid \gpMean(\Par), \likPar)$,
which ignores the emulator uncertainty. We can consider the ratio 
$\postApproxNormMarg(\Par)/\postApproxNormMean(\Par)$ to represent the uncertainty 
inflation factor for the EL approximation at a parameter $\Par$. This ratio can be above 
