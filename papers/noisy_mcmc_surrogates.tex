...
generative model in \Cref{eq:ep-approx}. The following result shows that 
...
\end{equation}
and expected $L_2$ loss
\begin{equation}
\mathcal{D}(\postNormEm, q) = \emE \left[\norm{\postNormEm - q}_{L_2(\parSpace)}\right].
\end{equation}
\end{prop}
...
For the below proofs we use the following measure-theoretic setup. We drop the 
$\Ndesign$ subscripts used throughout the paper for brevity.
