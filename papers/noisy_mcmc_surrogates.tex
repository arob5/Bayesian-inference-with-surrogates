...
MCMC algorithm that produces approximate EP samples. We focus on MCMC schemes
...
$\propPar \sim \propDistPar(\Par, \cdot)$. The next state $\Par^\prime$ is set to $\propPar$ with probability
\begin{equation}
...
kernel $\MarkovKerPar(\Par, \cdot; \targetTraj)$ defined in the previous section. The $\targetTraj$ update must leave 
...
\end{equation}
This MwG algorithm generally cannot be practically implemented due to the intractable normalizing 
