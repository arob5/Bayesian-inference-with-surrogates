...
model (e.g., GPs, BART, Bayesian neural networks, deep ensembles).
...
a particular emulator realization $\targetTraj$.
...
\end{align*}
In this setting, the EP assumes the form 
\begin{align*}
...
arises as an approximation to a coherent joint Bayesian model, showing
in this setting the EP can be viewed as a so-called cut posterior distribution
...
\end{align*}
In contrast with \Cref{eq:lik-approx-EP}, the EUP likelihood approximation 
...
in regions with high surrogate uncertainty. At two points where $\gpMean$ is equal, the 
EUP always assigns higher density to the more uncertain location. Notably, this uncertainty 
inflation scales with the exponentiated variance of the GP, potentially leading to extreme 
...
\citet{VehtariParallelGP}, can be viewed partially as a consequence \Cref{prop:ep-eup-pw-err}.
As illustrated in the experiments, the EP tends to be more robust, though not immune, to the 
...
We next return to \Cref{ex:fwd-em}, with surrogate density 
...
at $\Par$. The same increase in $\gpKer(\Par)$ may inflate the density at values 
...
To circumvent the challenges posed by the direct sampling algorithm, we present a Metropolis-Hastings (MH)
MCMC algorithm that produces approximate samples from the EP. We focus on MCMC schemes
that target the joint distribution $\emDist(d\targetTraj) \postDens(\Par; \targetTraj)/\normCst(\targetTraj) d\Par$, 
...
\end{align}
where $\MarkovKerTarget$ and $\MarkovKerPar$ are Markov kernels defined on $\emSpace$ and $\parSpace$,
respectively. We refer to algorithms of this form as \textit{random kernel MCMC}, reflecting that \Cref{eq:u-update}
modifies a typical, surrogate-free update $\MarkovKerPar(\Par, \cdot \given \target)$ with a kernel dependent 
on a random surrogate trajectory $\targetTraj^\prime$.

Throughout this section we fix $\MarkovKerPar$ to be an MH kernel defined with respect to a 
proposal kernel $\propDistPar$. The $\Par$-update in \Cref{eq:u-update} thus proceeds by first sampling a proposal 
$\propPar \sim \propDistPar(\Par, \cdot)$. The updated value $\Par^\prime$ is set to $\propPar$ with probability
\begin{equation}
...
\label{eq:MH-ratio-u},
\end{equation}
where $\propDensPar(\Par, \cdot)$ denotes the density of $\propDistPar(\Par, \cdot)$. Otherwise, it is set to 
the current value $\Par$. Provided the unnormalized density $\postDens(\Par; \targetTraj^\prime)$
depends on $\targetTraj$ only through $\targetTraj^\prime(\Par)$, then the MH update for $\Par$ only requires 
...
As a concrete starting point, we consider a Metropolis-within-Gibbs (MwG) algorithm to sample from 
the joint $\emDist(d\targetTraj) \postDens(\Par; \targetTraj)/\normCst(\targetTraj) d\Par$. The MH 
update for $\Par$ must leave $\postDens(\Par; \targetTraj) d\Par$ invariant, resulting in the MH 
kernel $\MarkovKerPar$ defined in the previous section. The $\targetTraj$ update must leave 
...
then we may take $\propDistTarget(\targetTraj, \cdot)$ to be the 
...
If it were possible to compute $\normCst(\targetTraj)$ then this could be turned into a practical
algorithm. As this is not the case, one option to address the intractable normalizing constant 
ratio is to apply techniques from the doubly intractable MCMC literature \citep{doublyIntractableReview}.
However, exact MCMC schemes such as the exchange algorithm \citep{exchangeAlg} require 
...
between $\targetTraj$ and $\tilde{\targetTraj}$. Therefore, choosing $\pcnCor$ close to one ensures 
that $\tilde{\targetTraj}$ is sufficiently similar to $\targetTraj$ on average. 

We emphasize that this algorithm can be implemented without discretizing $\targetTraj$ by tracking only the 
values at the current  and proposed states $(\Par, \propPar)$. As detailed in \issue{algorithm reference}, this is 
accomplished by ``peeking'' at the proposal $\propPar$ during the $\targetTraj$ update. The algorithm requires
the ability to condition the surrogate $\targetEm$ on at most a single new point each iteration. This avoids the 
repeated conditioning required by the direct sampling approach in \Cref{alg:ep}. See the \issue{appendix} for 
additional details regarding the efficient implementation of this algorithm.
\todo[inline]{Add section in appendix}
\todo[inline]{Clarify that the conditioning is done fresh each iteration? i.e., doesn't compound}
...
\begin{prop}
Under the linear Gaussian model \Cref{eq:lin-Gauss-inv-prob} with the emulator \Cref{eq:lin-Gauss-em},
...
exact posterior moments given in \Cref{eq:lin-Gauss-post-moments}.
\label{eq:lin-gaussi-ep}
\end{prop}
...
\label{eq:lin-gaussi-eup}
\end{prop}

\subsubsection{Analytical Analysis}
While both EP and EUP propagate uncertainty, their qualitative behavior is markedly 
different as the magnitude of the surrogate variance increases. From \Cref{eq:lin-gaussi-ep}, we see that the EP
is a mixture of linear Gaussian posteriors, each with covariance $\postCov$. The posterior inflation thus arises 
due to variability in the locations of each Gaussian in the mixture. On the other hand, from 
\Cref{eq:lin-gaussi-eup} the EUP arises from a data space adjustment of the original inverse problem, where
where the observed data is shifted by the emulator bias, and the observation covariance is inflated by the 
surrogate covariance. 


\subsubsection{SVD Analysis}
Consider the SVD $G = U S V^\top$ with singular values $s_j$ and consider the special case 
$\priorCov = c_0^2 I$, $\noiseCov = \sigma^2 I$. The exact posterior covariance $\postCov$ is diagonalized
in the $V$ basis, with $\postCov = V\Lambda V^\top$ where 
\begin{equation}
\lambda_j = \frac{c_0^2}{1 + \frac{c_0^2 s_j^2}{\sigma^2}}.
\end{equation}
The exact posterior mean $m$ can also be written with respect to this basis as
\begin{align}
m &= V\alpha + V_\perp \alpha_\perp \\
\alpha_j &= \frac{\lambda_j s_j}{\sigma_j^2} \langle y, u_j \rangle + \frac{\lambda_j}{c_0^2} \langle m_0, v_j \rangle, \qquad j = 1, \dots, r \\
(\alpha_\perp)_j &= \frac{\lambda_j}{c_0^2} \langle m_0, v_j \rangle, \qquad j = r+1, \dots, d
\end{align}
where $r$ is the rank of $G$.

\paragraph{Case 1: $Q = q^2 I$}

\begin{prop}
Assume $Q = q^2 I$. Then both $\postCovEUP$ and $\postCovEP$ are diagonalized in the $V$ basis,
with eigenvalues
\begin{align}
&\lambda_j^{\mathrm{EUP}} \Def \frac{c_0^2}{1 + \frac{c_0^2 s_j^2}{\sigma^2 + q^2}},
&&\lambda_j^{\mathrm{EP}} \Def \lambda_j + \frac{q^2 s_j^2 c_0^4}{\sigma^4}.
\end{align}
\end{prop}
Note that these eigenvalues correspond to the posterior variance along the respective $v_j$ directions.
We see that as $q \to \infty$, $\lambda_j^{\mathrm{EUP}} \to c_0^2$ and $\lambda_j^{\mathrm{EP}} \to \infty$.
In words, the EUP posterior variance is bounded by the prior while the EP variance is unbounded.
The uncertainty inflation is small for both EP and EUP in poorly informed directions in parameter space 
($v_j$ with $s_j \approx 0$). The inflation may be quite large in well-informed directions. 

\begin{prop}
Assume $Q = q^2 I$. Then $\postMeanEUP = V\alpha^{\mathrm{EUP}} + V_\perp \alpha_\perp $ and 
$\postMeanEP = V\alpha^{\mathrm{EP}} + V_\perp \alpha_\perp$, where
\begin{align}
\alpha_j^{\mathrm{EUP}} 
&\Def \frac{\lambda_j s_j}{\sigma^2 + q^2} \langle y - r, u_j \rangle + \frac{\lambda_j}{c_0^2} \langle m_0, v_j \rangle, \\
&= \left[\frac{\lambda_j s_j}{\sigma^2 + q^2} \langle y, u_j \rangle + \frac{\lambda_j}{c_0^2} \langle m_0, v_j \rangle \right]
- \frac{\lambda_j s_j}{\sigma^2 + q^2} \langle r, u_j \rangle \\
\alpha_j^{\mathrm{EP}} &\Def \alpha_j - \frac{c_0^2 s_j}{\sigma^2} \langle r, u_j \rangle.
\end{align}
\end{prop}

If the emulator bias $r$ is zero, then $\postMeanEP = \postMean$. This is not the case with the EUP, in which 
case $\postMeanEUP$ will still be tugged towards $m_0$. Combining this with the covariance results, it follows that 
in the regime $r = 0$ and $q \to \infty$,
\begin{align}
&\postApproxEUPNorm \to \Gaussian(\priorMean, \priorCov), &&\postApproxEP\to \Gaussian(\postMean, \infty).
\end{align}

The bias in $\postMeanEP$ depends only on $r$, not $q$. On the other hand, the bias in $\postMeanEUP$ depends 
both on $q$ and $r$. In both cases, the posterior mean will be more biased when the emulator bias $r$ is aligned
with directions that are well-informed by the data. 

% In the regime $r > 0$ and $q \to 0$ we have 

% Ecosystem Model Calibration
\subsection{Ecosystem Model Calibration}
...
structure of more complex land surface models \citep{vsem}. The model 
...
via the basic Euler scheme implemented in the R \verb+BayesianTools+ package \citep{vsem}. 
...
and assume independent uniform priors. Synthetic data $\obs$ is simulated
using \Cref{eq:vsem-inv-prob} with fixed ground truth values $\{\Par_{\star}, \sigma^2_{\star}\}$. 
