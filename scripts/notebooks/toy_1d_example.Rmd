...
save_output <- FALSE
output_dir <- file.path(base_dir, "output", "plots", "toy_1d", "efi2025")
...
pred_dt_fwd[, `:=`(em_mean=drop(pred_em_fwd$mean), 
                   ...
                   llik_mean=pred_llik_fwd$mean - shift,
                   llik_sd=sqrt(pred_llik_fwd$var),
                   lik_mean=pred_lik_fwd$log_mean - shift,
                   lik_sd=0.5 * pred_lik_fwd$log_var - shift)]
...
pred_em_llik <- em_llik$predict_emulator(input_grid)
pred_lik_llik <- em_llik$predict_lik(input_grid, em_pred_list=pred_em_llik, log_scale=TRUE)

pred_dt_llik[, `:=`(llik_mean=pred_em_llik$mean,
                    llik_sd=sqrt(pred_em_llik$var),
...
```{r}
# Single acquisition.
idx_acq <- which.max(pred_dt_fwd$post_sd)
u_acq <- pred_dt_fwd$input[idx_acq]
fwd_acq <- fwd(u_acq)
design_dt <- rbindlist(list(design_dt, list(input=u_acq, fwd=fwd_acq, llik=NA, lprior=NA, lpost=lpost(u_acq))))

# Update emulator.
em_fwd$emulator_model$update(matrix(u_acq, nrow=1L), matrix(fwd_acq, nrow=1L))
...
# Save plot.
plt_post_fwd_new <- ggplot(pred_dt_fwd, aes(x=input)) +
                     geom_line(aes(y=exp(lpost)), color="black") +
                     geom_line(aes(y=exp(post_mean)), color="blue") +
                     geom_ribbon(aes(ymin=post_lower, ymax=post_upper), 
                                 fill="skyblue", alpha=0.4) +
...
plt_post_fwd_new <- ggformat_journal(plt_post_fwd_new, xlim=lims_llik$xlim,
                                     ylim=c(0, 0.5)) + ylab("p(u|y)")
ggsave(file.path(output_dir, "post_dist_fwdem_acq1.png"), plt_post_fwd_new)
...
```{r}
# Another acquisition acquisition.
idx_acq <- which.max(pred_dt_fwd$post_sd)
u_acq <- pred_dt_fwd$input[idx_acq]
fwd_acq <- fwd(u_acq)
design_dt <- rbindlist(list(design_dt, list(input=u_acq, fwd=fwd_acq, llik=NA, lprior=NA, lpost=lpost(u_acq))))
...
plt_post_fwd_new <- ggformat_journal(plt_post_fwd_new, xlim=lims_llik$xlim,
                                     ylim=c(0, 0.5)) + ylab("p(u|y)")
ggsave(file.path(output_dir, "post_dist_fwdem_acq2.png"), plt_post_fwd_new)
...
plt_post_fwd_new <- ggformat_journal(plt_post_fwd_new, xlim=lims_llik$xlim,
                                     ylim=c(0, 0.5)) + ylab("p(u|y)")
ggsave(file.path(output_dir, "post_dist_fwdem_acq3.png"), plt_post_fwd_new)
...
plt_post_fwd_new <- ggformat_journal(plt_post_fwd_new, xlim=lims_llik$xlim,
                                     ylim=c(0, 0.5)) + ylab("p(u|y)")
ggsave(file.path(output_dir, "post_dist_fwdem_acq4.png"), plt_post_fwd_new)
...
lik_approx_fwd_dt[,`:=`(mean=mean-shift, marginal=marginal-shift)]
...
color_map <- c(post_norm="black", post_norm_mean="#0072B2", post_norm_marg="#E69F00",
               "mcwmh-joint"="#CC79A7", "mcwmh-ind"="#009E73")
label_map <- c(post_norm="exact", post_norm_mean="mean", post_norm_marg="EL",
               "mcwmh-joint"="mcwmh-joint", "mcwmh-ind"="mcwmh-ind")
...
# Align axes for plots. All plots will use `lims_llik$xlim` for the x-axis.
lims_post_norm_approx <- get_common_lims(post_approx_plt_fwd, post_approx_plt_llik)

# Format plots.
post_approx_plt_fwd <- ggformat_journal(post_approx_plt_fwd, 
                                        xlim=lims_post_norm_approx$xlim,
                                        ylim=lims_post_norm_approx$ylim)
post_approx_plt_llik <- ggformat_journal(post_approx_plt_llik, 
                                         xlim=lims_post_norm_approx$xlim,
                                         ylim=lims_post_norm_approx$ylim,
                                         legend_position="bottom",
...
# Re-save trimmed versions of lpost distribution plots.
lpost_bounds <- c(-10, 1)
